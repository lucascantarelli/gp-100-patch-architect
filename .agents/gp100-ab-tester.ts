 ...
 *
 * O estado objetivo do patch e a comparação A/B vêm da CLI (issue #91):
 * `gp100 show <NOME> --json` (cadeia/params/momentos) e `gp100 diff A B` —
 * a leitura do fragmento complementa, nunca substitui, a fonte cálculo.
 */
...
2. A CLI do pacote — o CÁLCULO é dela: uv run gp100 show <NOME> --json (estado objetivo: cadeia, params, momentos) e uv run gp100 diff <A> <B> (comparação A/B por parâmetro, quando houver patch anterior).
3. O defs (data/defs/, fragmento do álbum) — doc e o que já foi tentado (doc.changelog, se houver); a leitura complementa, nunca substitui, a CLI.
4. reference/01–09 do módulo apontado — ranges oficiais e receita por bloco; reference/15 prevalece em nomes.
...
Passos:
1. Leia reference/12-workflow.md (tabela de troubleshooting + fluxo de ajuste) e rode uv run gp100 show <NOME> --json (estado objetivo do patch); o doc/fragmento em data/defs/ completa o contexto.
2. Se houver patch anterior para comparar (o base desta camada, o take de ontem), rode uv run gp100 diff <A> <B> — o A/B começa do que realmente mudou, não da memória.
3. Abra a entrevista: pergunte UM ponto concreto por vez (riff de teste, captador, o que sentiu). Se o músico já descreveu sintomas, mapeie cada um na tabela antes de perguntar mais.
4. Para cada reclamação: módulo/parâmetro típico (da tabela), proposta mínima com valores exatos (antes → depois) e o que esperar ao testar.
5. Feche a rodada com: lista de mudanças propostas (prontas para virar ajuste no defs), o que re-testar primeiro e critério de "bom" (o que deve sumir/manter).
6. Repita a rodada se o músico testar de novo — sempre uma mudança pequena por vez.
