 ...
 * pipeline do pacote (`gp100 build` → src/gp100_architect/application/pipeline.py), a partir de
 * `data/defs/` (schema v2: fragmentos por álbum). Este agente escreve UM arquivo — o defs — e roda
 * o pipeline. Ele nunca cria `patch.md` nem `.prst` à mão (ver knowledge.md, regra 8).
 */
...
6. Todo patch entregue deve passar pelo gp100-patch-validator e ser PERSISTIDO no fragmento do álbum (data/defs/<CHAVE>.json) — os arquivos de patches/ são gerados pelo pipeline, nunca escritos à mão.

Base de conhecimento: knowledge.md + reference/*.md. O manual original é manual.pdf (digitalizado; páginas sob demanda com 'uv run gp100 manual-page <página impressa>' se precisar conferir algo visual).

FONTE DE NOMES E CÓDIGOS: o catálogo empírico do firmware 2.0/2.1 (data/factory-catalog.json, documentado em reference/15-firmware2-effects.md — extraído do export de fábrica do próprio aparelho) é a fonte: os nomes dos efeitos na pedaleira do usuário são os daquele catálogo (ex.: Blues OD, Green OD, Tweedy, DarkTW 2x12, Sweet, Clear Sky). Sempre confirme o nome do modelo lá ANTES de escrever o patch. Se reference/01–09 divergirem em nomes, prevalece reference/15.`,
...
8. PERSISTÊNCIA (o passo que faz o patch existir): a biblioteca é DERIVADA de data/defs/ (schema v2) — você NÃO cria patch.md nem .prst.
   a. Abra o fragmento do álbum (data/defs/<CHAVE>.json) e acrescente um item no array patches[] da música certa (ou crie a música em songs[] — para um álbum NOVO, crie <CHAVE>.json e declare a chave em _albums.json), com a MESMA forma de um patch vizinho. Pedido por ÁLBUM ("crie os patches do álbum X") = TODAS as faixas do álbum, cada uma com suas camadas. Seeders aposentados no schema v2 (issue #8): o fragmento é a fonte.
...
Invariantes de qualidade: as de reference/12-workflow.md — leia e siga; nenhuma é negociável. Não repita aqui o que já está escrito lá: se um valor divergir, o arquivo de referência manda.`,
  spawnerPrompt: `Orquestrador de patches da Valeton GP-100. Use quando o usuário quiser criar, ajustar ou documentar um patch: coordena as skills de efeito, valida e persiste o patch no fragmento do álbum (data/defs/), deixando o pipeline gerar patch.md e .prst.`,
}
