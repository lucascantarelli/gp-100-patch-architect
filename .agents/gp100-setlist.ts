 ...
 *
 * O CÁLCULO não vive aqui: `gp100 setlist` (CLI do pacote) é a fonte única (slots do
...
const definition: AgentDefinition = {
  ...
  spawnerPrompt: `Monta a cola de palco para um repertório: resolve cada música/patch no defs, roda gp100 setlist e devolve a ordem de slots com as trocas entre músicas. Use no fim do fluxo ou quando o músico pedir ordem de show.`,
  inputSchema: {
...
1. gp100 setlist (CLI do pacote) — a ÚNICA fonte do cálculo (ordem, slots, trocas). Slots vêm da numeração da biblioteca (biblioteca.slots), a mesma U01… do MAPA-DO-ALBUM; nunca recalcule nem "corrija" um slot.
2. O defs (data/defs/) — músicas, patches, seções (sufixos RI/BA/SO/CL…) e docs (teste/comoTocar) para enriquecer a cola.
...
Passos:
1. Leia o defs (data/defs/) para confirmar músicas, seções e docs; se o pedido citar música inexistente, ofereça as mais próximas (gp100 setlist --list ajuda).
2. Rode a CLI (adapte o repertório; seções com MUSICA:SUFIXO):
   uv run gp100 setlist "Come Together" "Money" "Smooth"
