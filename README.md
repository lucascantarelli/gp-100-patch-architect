...
<!-- Badges DERIVADOS: gerados por `gp100 badges` e publicados no Pages —
     não edite contagens à mão; rode o comando (a URL já aponta para o
     badges.json do deploy). Os demais (CI, CodeQL, License…) vêm de serviços. -->
[![Release](https://img.shields.io/endpoint?url=https%3A%2F%2Flucascantarelli.github.io%2Fgp-100-patch-architect%2Fbadges%2Frelease.json&style=flat-square)](https://github.com/lucascantarelli/gp-100-patch-architect/releases)
[![Firmware](https://img.shields.io/badge/firmware-2.1%20(confirmado%20no%20device)-2ea44f?style=flat-square)](reference/15-firmware2-effects.md)
[![Agentes](https://img.shields.io/endpoint?url=https%3A%2F%2Flucascantarelli.github.io%2Fgp-100-patch-architect%2Fbadges%2Fagentes.json&style=flat-square)](.agents/README.md)
[![Skills](https://img.shields.io/endpoint?url=https%3A%2F%2Flucascantarelli.github.io%2Fgp-100-patch-architect%2Fbadges%2Fskills.json&style=flat-square)](.agents/README.md)
[![Patches](https://img.shields.io/endpoint?url=https%3A%2F%2Flucascantarelli.github.io%2Fgp-100-patch-architect%2Fbadges%2Fpatches.json&style=flat-square)](https://lucascantarelli.github.io/gp-100-patch-architect/)
[![Formato](https://img.shields.io/badge/.prst-single%20fw%202.1-2ea44f?style=flat-square)](docs/decisions/0013-modelo-de-artefatos-em-escala.md)
[![Python](https://img.shields.io/endpoint?url=https%3A%2F%2Flucascantarelli.github.io%2Fgp-100-patch-architect%2Fbadges%2Fpython.json&style=flat-square)](pyproject.toml)
[![Testes](https://img.shields.io/endpoint?url=https%3A%2F%2Flucascantarelli.github.io%2Fgp-100-patch-architect%2Fbadges%2Ftestes.json&style=flat-square)](pyproject.toml)
[![Cobertura](https://img.shields.io/endpoint?url=https%3A%2F%2Flucascantarelli.github.io%2Fgp-100-patch-architect%2Fbadges%2Fcobertura.json&style=flat-square)](pyproject.toml) [![Site](https://img.shields.io/badge/📚%20biblioteca%20online-lucascantarelli.github.io-2f6fdd?style=flat-square)](https://lucascantarelli.github.io/gp-100-patch-architect/) [![CI](https://github.com/lucascantarelli/gp-100-patch-architect/actions/workflows/ci.yml/badge.svg)](https://github.com/lucascantarelli/gp-100-patch-architect/actions/workflows/ci.yml) [![Security](https://github.com/lucascantarelli/gp-100-patch-architect/actions/workflows/security.yml/badge.svg)](https://github.com/lucascantarelli/gp-100-patch-architect/actions/workflows/security.yml) [![CodeQL](https://img.shields.io/badge/CodeQL-Python%20·%20TypeScript-2f6fdd?style=flat-square)](https://github.com/lucascantarelli/gp-100-patch-architect/security/code-scanning) [![License: MIT](https://img.shields.io/badge/license-MIT-2ea44f?style=flat-square)](LICENSE) [![PRs welcome](https://img.shields.io/badge/PRs-welcome-e02d2d?style=flat-square)](CONTRIBUTING.md)
...
| `gp100 badges` | `uv run gp100 badges [--destino site]` | **Badges derivados** (issue #117): `badges/*.json` (endpoint do shields.io) com release, agentes, skills, patches, Python, cobertura e testes — contagens de fonte única (`VERSION`, `git ls-files`, defs, pyproject, coleção pytest), geradas no deploy do Pages. Os badges do README consomem as URLs do Pages: dado derivado não se edita, se regenera |
| `gp100 site` | `uv run gp100 site [--destino dist/site] [--base-url /gp-100-patch-architect/]` | **Site estático da biblioteca** (issue #11) **+ catálogo JSON** (issue #90 fase 1): página por álbum e por patch + busca client-side sobre `busca.json` minúsculo — derivado do defs, sem backend. **Publicado no GitHub Pages a cada push na `develop`**: [lucascantarelli.github.io/gp-100-patch-architect](https://lucascantarelli.github.io/gp-100-patch-architect/) · catálogo em `/catalog/` (`index`, `patches/<NOME>`, `irs`, `schema`) |
...
|---|---|
| [`ci.yml`](.github/workflows/ci.yml) | Dados em sincronia + suíte completa + typecheck dos agentes (contagem no badge derivado) (Python 3.14 fixado e travado no job) |
...
| [`pages.yml`](.github/workflows/pages.yml) | **Um deploy, dois sites**: biblioteca na raiz + docs de engenharia em `/docs/` — derivados do checkout fresco a cada push na `develop` |
| [`project-automation.yml`](.github/workflows/project-automation.yml) | Board (Project #7): entra/move cards, guardian de PR e relatório de milestone — escrita via PAT, nunca pelo `GITHUB_TOKEN` |

> O antigo `docs-pages.yml` foi **unificado no `pages.yml`** (auditoria pré-2.0.1): o `deploy-pages@v4` não tem input de subcaminho — dois deploys no mesmo Pages eram last-writer-wins, e o último derrubava o site do outro.
