...
tools/                 ← build_song_patches.py (gera patches por música) · generate_prst.py (gera .prst single) · gen_indexes.py (regenera índices) · ir_library.py (indexa IRs) · check_data_freshness.py (guarda do CI: disco × HEAD) · render_manual_page.py (páginas do manual sob demanda) · analyze_prst.py (disseca exports) · patches-defs.json (FONTE DOS PATCHES)
prompts/               ← templates de pedido do usuário (incl. criação por referência de artista)
templates/             ← template de documentação de patch
patches/               ← biblioteca: BANDA/ÁLBUM/MÚSICA/MUSICA-CAMADA (patch.md + .prst + spec.json)
...
12. **Testes e dados em sincronia (obrigatório antes de entregar qualquer mudança)**: `python -m unittest discover -s tests -v` — a suíte valida o formato `.prst` single fw 2.1, as 9 seções da doc, os momentos de toggle, a coerência mapa × `patch.md` sobre IR, a cobertura de nomes de parâmetro e o drift dos índices. **Dado gerado nunca vai "de mão": se você acrescentou música, camada, patch, efeito, momento de toggle ou pack de IR, rode o pipeline inteiro e commite os derivados** — `python tools/ir_library.py` → `python tools/add_pulse_defs.py` (já encadeia `add_momentos.py`) → `python tools/build_song_patches.py` → `python tools/gen_indexes.py` → `python tools/check_data_freshness.py` (este último reprova se o disco não bater com o HEAD, ignorando só o `preset_info/@time`). O CI (`.github/workflows/ci.yml`) roda a cada push/PR em **três jobs paralelos** — **`data-pipeline`** (pipeline inteiro + check de frescor — e, **em push no `main`, commita e envia os dados regenerados sozinho**, deixando o repositório sempre em sincronia), **`test-suite`** (compile + suíte) e **`typecheck`** (`tsc --noEmit`) — seguidos do portão **`ci-gate`**, que reprova se qualquer um falhar.
...
- Interativo: `@gp100-patch-architect quero um patch de blues comdelay pra tocar com backing em casa`
