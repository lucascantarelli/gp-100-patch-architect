...
12. **Testes e dados em sincronia (obrigatório antes de entregar qualquer mudança)**: `python -m unittest discover -s tests -v` — a suíte valida o formato `.prst` single fw 2.1, as 9 seções da doc, os momentos de toggle, a coerência mapa × `patch.md` sobre IR, a cobertura de nomes de parâmetro, o drift dos índices e o **guarda de sincronia**: o `TestH` roda o pipeline inteiro numa cópia temporária do repositório e reprova se o resultado não reproduzir exatamente o commitado (ignorando só o `preset_info/@time`) — pega "esqueci de rodar o pipeline" e hand-edit em arquivo gerado. **Dado gerado nunca vai "de mão": se você acrescentou música, camada, patch, efeito, momento de toggle ou pack de IR, rode o pipeline inteiro e commite os derivados** — `python tools/ir_library.py` → `python tools/add_pulse_defs.py` (já encadeia `add_momentos.py`) → `python tools/build_song_patches.py` → `python tools/gen_indexes.py`. O CI (`.github/workflows/ci.yml`) roda a cada push/PR em **dois jobs paralelos** — **`test-suite`** (compile + suíte, incluindo o guarda de sincronia; nenhum job escreve no repositório) e **`typecheck`** (`tsc --noEmit`) — seguidos do portão **`ci-gate`**, que reprova se qualquer um falhar.
