...
> 💡 **Casino de John direto no canal da mesa: distorção áspera de rádio, sem amp modelado, cabine JBL derrapando**
...
1. Palm-mute leve nos tempos fortes do riff; abra a palheta no 'come toge-ther'
2. Ataque determinado — a sujeira vem da 'mesa clipada', não de força extra

> 🚀 **Comece por aqui (3 passos)**:
> 1. Ajuste a guitarra conforme a tabela acima.
> 2. Carregue o patch no slot **U01** (seção 8).
> 3. Toque *riff principal + virada para D (última parte do verso)* e confira o checklist da seção 5.
...
|---|---|
| raspando demais | DST Tone +3 |
| mole no ataque | DST Gain +3 |
| harmônico agudo irritando | CAB High Cut 55 |
...
**Nota específica deste patch**: Alvo = mesa derrapada da Abbey Road, não cabine real — nenhuma IR do banco casa; mantenha o CAB de fábrica.
...
**O que este patch é**: Casino de John direto no canal da mesa: distorção áspera de rádio, sem amp modelado, cabine JBL derrapando.
...
**Teste recomendado**: riff principal + virada para D (última parte do verso) · **Drum**: Rock 104 BPM

**Como saber que está certo**:

- ☑️ raspado agressivo sem embolar o grave
- ☑️ harmônicos do RAT presentes mas não gritando
...
|---|---|---|---|---|---|---|---|---|
| ~~⚪~~ PRE | **🔴** DST · La Charger | ~~⚪~~ AMP | **🔴** NR · Gate 1 | **🔴** CAB · J-120 2x12 | **🔴** EQ · EQ 1 | ~~⚪~~ MOD | ~~⚪~~ DLY | **🔴** RVB · Room |
...
|---|---|---|
| DST | La Charger | Gain: 28 · Tone: 45 · Volume: 62 |
| NR | Gate 1 | Thr: 26 |
...
**Nome no painel**: `CT01RIF` · **Slot sugerido**: **U01**

1. **GP-100 Edits (recomendado)**: conecte a GP-100 por USB → importe `<NOME>.prst` desta pasta → salve no slot **U01**.
2. **Digitar no painel** (receita na ordem dos menus):

```
**DST** `La Charger` (Gain 28 / Tone 45 / Volume 62) → **NR** `Gate 1` (Thr 26) → **CAB** `J-120 2x12` (Level 75 / High Cut 50) → **EQ** `EQ 1` (Low -1 / Mid 2 / High -1 / Level 50) → **RVB** `Room` (ajuste fino no painel — seção 3 📡)
```

3. **SAVE** no slot → renomeie para `CT01RIF`.

---

## 🚫 9. Evite com este patch

- ⛔ Ligar o AMP para 'engordar' — na gravação não há amp
- ⛔ Tone do La Charger acima de 60 — vira fuzz ácido
