# 👑 GS101BA
### Golden Slumbers / Carry That Weight — Base
...
> 💡 **Do violão simulado (Golden Slumbers intro) ao peso do medley: AC Sim + foxy crunch com controle de dinâmica**

---

## 🎸 1. Sua guitarra agora — leia isto primeiro

> **⚡ Ajuste a Strat antes de tocar: Seletor 5 (neck) · Volume 8 · Tone 6 · use o Volume como pedal de dinâmica**

| Controle na guitarra | Ajuste |
|---|---|
| **Seletor de captadores** | Posição 5 — neck com Tone 6 |
| **Volume** | 8 |
| **Tone** | 6 |

**🎯 Técnica que completa o som** (a dinâmica da mão vale tanto quanto os parâmetros):

1. Golden Slumbers: ataque suave (a seção é quase limpa)
2. Carry That Weight: abra tudo — use o knob de Volume da guitarra como pedal

> 🚀 **Comece por aqui (3 passos)**:
> 1. Ajuste a guitarra conforme a tabela acima.
> 2. Carregue o patch no slot **U18** (seção 8).
> 3. Toque *'Golden Slumbers' inteira → 'Carry That Weight'* e confira o checklist da seção 5.
...
|---|---|
| Golden limpa demais | foxy Vol +4 |
| Carry mole | DST Gain +3 |
...
**Nota específica deste patch**: AC30 — captura local opcional (User IR 2); a fábrica já entrega o alvo.
...
**O que este patch é**: Do violão simulado (Golden Slumbers intro) ao peso do medley: AC Sim + foxy crunch com controle de dinâmica.

Golden Slumbers: Paul no Casino (limpo com leve crujido). Carry That Weight: guitarras em peso no medley. Dois momentos, um patch base.

**Teste recomendado**: 'Golden Slumbers' inteira → 'Carry That Weight' · **Drum**: Pop 96 BPM

**Como saber que está certo**:

- ☑️ contraste dinâmico claro entre as seções
...
|---|---|---|
| Paul: Casino, dinâmico de limpo a crujido | média | Guitar World |
...
|---|---|---|
| DST | Green OD | Gain: 38 · Tone: 52 · Level: 60 |
| AMP | Foxy 30TB | Vol: 60 · Cut: 35 · Master: 68 · Bass: 52 · Treble: 55 |
| NR | Gate 1 | Thr: 26 |
...
**Nome no painel**: `GS101BA` · **Slot sugerido**: **U18**

1. **GP-100 Edits (recomendado)**: conecte a GP-100 por USB → importe `<NOME>.prst` desta pasta → salve no slot **U18**.
2. **Digitar no painel** (receita na ordem dos menus):

```
**DST** `Green OD` (Gain 38 / Tone 52 / Level 60) → **AMP** `Foxy 30TB` (Vol 60 / Cut 35 / Master 68 / Bass 52 / Treble 55) → **NR** `Gate 1` (Thr 26) → **CAB** `Foxy 1x12` (Level 75 / High Cut 50) → **EQ** `EQ 1` (Low 0 / Mid 1 / High 0 / Level 50) → **RVB** `Room` (ajuste fino no painel — seção 3 📡)
```

3. **SAVE** no slot → renomeie para `GS101BA`.

---

## 🚫 9. Evite com este patch

- ⛔ Usar AC Sim ON junto com drive forte — fica artificial
