# 🔥 PTP01BA
### Polythene Pam / She Came In Through the Bathroom Window — Base
...
> 💡 **Crunch ágil do medley: Green OD + AC30 estufado, ataque seco e pronto**
...
1. Riffs rápidos em D e A com palhetada alternada — secos, sem sustain
2. Bathroom Window: acordes abertos com vibrato de dedo nas notas finais

> 🚀 **Comece por aqui (3 passos)**:
> 1. Ajuste a guitarra conforme a tabela acima.
> 2. Carregue o patch no slot **U17** (seção 8).
> 3. Toque *'Polythene Pam' completo + entrada de 'Bathroom Window'* e confira o checklist da seção 5.
...
**O que este patch é**: Crunch ágil do medley: Green OD + AC30 estufado, ataque seco e pronto.

Medley rock: Casino no AC30, riffs rápidos e secos. Bathroom Window tem acordes abertos vibrantes.

**Teste recomendado**: 'Polythene Pam' completo + entrada de 'Bathroom Window' · **Drum**: Rock 105 BPM

**Como saber que está certo**:

- ☑️ ataque seco e rápido
- ☑️ acordes abertos no fim
...
|---|---|---|
| John/Paul: Casino no AC30 | alta | Guitar World |
...
**Nome no painel**: `PTP01BA` · **Slot sugerido**: **U17**

1. **GP-100 Edits (recomendado)**: conecte a GP-100 por USB → importe `<NOME>.prst` desta pasta → salve no slot **U17**.
2. **Digitar no painel** (receita na ordem dos menus):

```
**DST** `Green OD` (Gain 45 / Tone 55 / Level 62) → **AMP** `Foxy 30TB` (Vol 68 / Cut 35 / Master 65 / Bass 50 / Treble 55) → **NR** `Gate 1` (Thr 28) → **CAB** `Foxy 1x12` (Level 75 / High Cut 50) → **EQ** `EQ 1` (Low 0 / Mid 1 / High 0 / Level 50) → **RVB** `Room` (ajuste fino no painel — seção 3 📡)
```

3. **SAVE** no slot → renomeie para `PTP01BA`.

---

## 🚫 9. Evite com este patch

- ⛔ Delay — o medley é corrido e seco
