...
> 💡 **Twin quase limpo com a borda quebrando no ataque: o lugar exato entre limpo e crujido da gravação**
...
|---|---|
| **Seletor de captadores** | Posição 4 — middle+neck (quack suave) |
| **Volume** | 7–8 |
| **Tone** | 8 |

**🎯 Técnica que completa o som** (a dinâmica da mão vale tanto quanto os parâmetros):

1. Ataque de palheta suave nos acordes — o quase-breakup vem do amp, não da mão
2. Volume 7 na intro e abra a 8 no refrão para dar respiro dinâmico

> 🚀 **Comece por aqui (3 passos)**:
> 1. Ajuste a guitarra conforme a tabela acima.
> 2. Carregue o patch no slot **U03** (seção 8).
> 3. Toque *intro C-D-E e primeira estrofe* e confira o checklist da seção 5.
...
|---|---|
| limpo demais | AMP Vol +3 |
| sujou | AMP Vol -3 |
| agudo estridente | AMP Treble -2 |
...
**Nota específica deste patch**: Recomendada: American Twin 2x12 Medium Mix (User IR 1) — é o Twin real da sessão, com o JBL D120F.
...
**O que este patch é**: Twin quase limpo com a borda quebrando no ataque: o lugar exato entre limpo e crujido da gravação.
...
**Teste recomendado**: intro C-D-E e primeira estrofe · **Drum**: Ballad 66 BPM

**Como saber que está certo**:

- ☑️ ataque quebra sutilmente sem sujar
- ☑️ corpo dos acordes intacto
...
**Nome no painel**: `STH01BA` · **Slot sugerido**: **U03**

1. **GP-100 Edits (recomendado)**: conecte a GP-100 por USB → importe `<NOME>.prst` desta pasta → salve no slot **U03**.
2. **Digitar no painel** (receita na ordem dos menus):

```
**AMP** `Dark Twin` (Vol 55 / Output 62 / Bass 52 / Middle 48 / Treble 55 / Bright Off) → **NR** `Gate 1` (Thr 24) → **CAB** `DarkTW 2x12` (Level 75 / High Cut 50) → **EQ** `EQ 1` (Low 0 / Mid 1 / High 0 / Level 50) → **RVB** `Room` (ajuste fino no painel — seção 3 📡)
```

3. **SAVE** no slot → renomeie para `STH01BA`.
...
- ⛔ Bright ON
- ⛔ Reverb longo — a sala da Abbey Road é curta
