...
1. Arpejo descendo, tempo largo — é hipnose
2. No refrão, raiva: palhetada total e para baixo

> 🚀 **Comece por aqui (3 passos)**:
> 1. Ajuste a guitarra conforme a tabela acima.
> 2. Carregue o patch no slot **U69** (seção 8).
> 3. Toque *intro/verso (quiet)* e confira o checklist da seção 5.
...
1. **Refrão pesado** — **DST → ON**
   *Quando*: A explosão do refrão é um stomp: DST ON = wall of sound.
   *Dica*: Volte ao quiet-loud desligando.

2. **Solo livre** — **MOD → OFF** + **DST → ON**
   *Quando*: Solo de fechamento: chorus sai, distorção entra.
...
**O que este patch é**: Quiet-loud: arpejo lento ("come on over, do the twist…") que explode no refrão.

Quiet-loud: arpejo quebrado no verso, explosão no refrão, solo de jam no final.

**Teste recomendado**: intro/verso (quiet) · **Drum**: 90 bpm quiet-loud

**Como saber que está certo**:

- ☑️ arpejo quebrado e hipnótico
- ☑️ explosão de refrão dá contraste total
...
**Nome no painel**: `ANE01AR` · **Slot sugerido**: **U69**

1. **GP-100 Edits (recomendado)**: conecte a GP-100 por USB → importe `<NOME>.prst` desta pasta → salve no slot **U69**.
...
3. **SAVE** no slot → renomeie para `ANE01AR`.
