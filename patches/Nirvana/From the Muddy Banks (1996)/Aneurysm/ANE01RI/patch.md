...
1. Palhetada total, dinâmica no braço
2. Deixe o Room curto dar a sala do show ao vivo

> 🚀 **Comece por aqui (3 passos)**:
> 1. Ajuste a guitarra conforme a tabela acima.
> 2. Carregue o patch no slot **U70** (seção 8).
...
1. **Verso limpo** — **DST → OFF** + **MOD → ON**
   *Quando*: O verso é arpejo limpo com chorus — um stomp só.
   *Dica*: Refrão volta com DST ON.
...
**O que este patch é**: O refrão inteiro de DS-1 no talo, bateria e baixo por cima.

Quiet-loud: arpejo quebrado no verso, explosão no refrão, solo de jam no final.

**Teste recomendado**: refrão · **Drum**: grunge 90 bpm

**Como saber que está certo**:

- ☑️ parede de som com ataque de palhetada
- ☑️ corte seco no fim da frase
...
**Nome no painel**: `ANE01RI` · **Slot sugerido**: **U70**

1. **GP-100 Edits (recomendado)**: conecte a GP-100 por USB → importe `<NOME>.prst` desta pasta → salve no slot **U70**.
2. **Digitar no painel** (receita na ordem dos menus):

```
**DST** `La Charger` (Gain 66 / Tone 48 / Volume 62) → **AMP** `Dark Twin` (Vol 55 / Output 62 / Bass 52 / Middle 48 / Treble 55 / Bright Off) → **NR** `Gate 1` (Thr 26) → **CAB** `DarkTW 2x12` (Level 75 / High Cut 50) → **RVB** `Room` (ajuste fino no painel — seção 3 📡) → SOBRESSALENTE **MOD** `A-Chorus` (Rate 30 / Depth 0.5 / Mix 50 / Level 0)
```

3. **SAVE** no slot → renomeie para `ANE01RI`.
