...
1. Frases dissonantes, bends furiosos
2. Não busque "bonito": busque tenso

> 🚀 **Comece por aqui (3 passos)**:
> 1. Ajuste a guitarra conforme a tabela acima.
> 2. Carregue o patch no slot **U76** (seção 8).
...
1. **Solo espectral** — **EQ → ON**
   *Quando*: Realce para o solo curto e doente.
...
**O que este patch é**: Solo: curto, doente e espectral — pura atitude.

Base punk rápida e rasgada com solo curto e dissonante (Paradiso 1991).

**Teste recomendado**: solo central · **Drum**: punk 160 bpm

**Como saber que está certo**:

- ☑️ lead tenso e dissonante
- ☑️ atitude por cima da base rápida
...
|---|---|---|
| DST | La Charger | Gain: 68 · Tone: 55 · Volume: 62 |
...
**Nome no painel**: `BEE01SO` · **Slot sugerido**: **U76**

1. **GP-100 Edits (recomendado)**: conecte a GP-100 por USB → importe `<NOME>.prst` desta pasta → salve no slot **U76**.
2. **Digitar no painel** (receita na ordem dos menus):

```
**DST** `La Charger` (Gain 68 / Tone 55 / Volume 62) → **AMP** `Dark Twin` (Vol 55 / Output 62 / Bass 52 / Middle 48 / Treble 55 / Bright Off) → **NR** `Gate 1` (Thr 26) → **CAB** `DarkTW 2x12` (Level 75 / High Cut 50) → **RVB** `Room` (ajuste fino no painel — seção 3 📡) → SOBRESSALENTE **EQ** `EQ 1` (Mid 3 / Level 56)
```

3. **SAVE** no slot → renomeie para `BEE01SO`.
