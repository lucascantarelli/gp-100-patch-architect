...
> **⚡ Ajuste a Strat antes de tocar: SELETOR 3 · Volume 10 · Tone 7 (o fuzz escurece o grave da Strat)**
...
1. Riff lento e hipnótico, com muito sustain
2. O fuzz sustenta: deixe as notas morrerem sozinhas

> 🚀 **Comece por aqui (3 passos)**:
> 1. Ajuste a guitarra conforme a tabela acima.
> 2. Carregue o patch no slot **U92** (seção 8).
...
1. **Solo psicodélico** — **EQ → ON**
   *Quando*: Solo final: realce de presença para os harmônicos sobre o fuzz.
...
**O que este patch é**: O riff em Em com FUZZ: o som do desespero — _Red Haze no lugar do DS-1_.

O fecho: riff com fuzz (Red Haze) e solo psicodélico — o momento stoner da banda (Paradiso 1991).

**Teste recomendado**: riff principal · **Drum**: grunge 98 bpm

**Como saber que está certo**:

- ☑️ fuzz denso e encorpado
- ☑️ sustain longo nas notas terminais
...
**Nome no painel**: `BLE01RI` · **Slot sugerido**: **U92**

1. **GP-100 Edits (recomendado)**: conecte a GP-100 por USB → importe `<NOME>.prst` desta pasta → salve no slot **U92**.
2. **Digitar no painel** (receita na ordem dos menus):

```
**DST** `Red Haze` (Fuzz 62 / VOL 45) → **AMP** `Dark Twin` (Vol 55 / Output 62 / Bass 52 / Middle 48 / Treble 55 / Bright Off) → **NR** `Gate 1` (Thr 26) → **CAB** `DarkTW 2x12` (Level 75 / High Cut 50) → **RVB** `Room` (ajuste fino no painel — seção 3 📡) → SOBRESSALENTE **EQ** `EQ 1` (template de fábrica — ajuste por orelha ao ligar)
```

3. **SAVE** no slot → renomeie para `BLE01RI`.
