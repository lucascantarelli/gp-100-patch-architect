...
1. Riff com pestana na 6ª corda, dinâmica no braço
2. Deixe a distorção lo-fi embolar as notas graves

> 🚀 **Comece por aqui (3 passos)**:
> 1. Ajuste a guitarra conforme a tabela acima.
> 2. Carregue o patch no slot **U90** (seção 8).
...
|---|---|
| graves virando lama | CAB High Cut +2 |
| falta mordida | Tone +4 |
...
**O que este patch é**: Riff com raiz grave: a versão aqui é mais lenta que o disco (era "Imodium").

A "Imodium" de 1989: mais lenta que o disco, riff com raiz grave.

**Teste recomendado**: riff principal · **Drum**: grunge 140 bpm

**Como saber que está certo**:

- ☑️ riff grave e embolado
- ☑️ energia bruta sem definição cirúrgica
...
|---|---|---|
| DST | La Charger | Gain: 68 · Tone: 40 · Volume: 62 |
...
**Nome no painel**: `BRE01RI` · **Slot sugerido**: **U90**

1. **GP-100 Edits (recomendado)**: conecte a GP-100 por USB → importe `<NOME>.prst` desta pasta → salve no slot **U90**.
2. **Digitar no painel** (receita na ordem dos menus):

```
**DST** `La Charger` (Gain 68 / Tone 40 / Volume 62) → **AMP** `Dark Twin` (Vol 55 / Output 62 / Bass 52 / Middle 48 / Treble 55 / Bright Off) → **NR** `Gate 1` (Thr 26) → **CAB** `DarkTW 2x12` (Level 75 / High Cut 50) → **RVB** `Room` (ajuste fino no painel — seção 3 📡) → SOBRESSALENTE **EQ** `EQ 1` (template de fábrica — ajuste por orelha ao ligar)
```

3. **SAVE** no slot → renomeie para `BRE01RI`.
