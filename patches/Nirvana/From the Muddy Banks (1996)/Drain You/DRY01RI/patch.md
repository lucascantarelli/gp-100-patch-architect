...
> **⚡ Ajuste a Strat antes de tocar: SELETOR 3 · Volume 10 · Tone 10**
...
1. Acordes cheios, palhetada total (6 cordas), para baixo
2. Intensidade vem do braço: o ganho já está no teto útil

> 🚀 **Comece por aqui (3 passos)**:
> 1. Ajuste a guitarra conforme a tabela acima.
> 2. Carregue o patch no slot **U67** (seção 8).
> 3. Toque *refrão ("one baby to another says I'm lucky…")* e confira o checklist da seção 5.
...
|---|---|
| embolando demais | seletor 3 · Tone 8 |
| grilos com ganho alto | NR Thr -2 |
...
1. **Verso limpo** — **DST → OFF** + **MOD → ON**
   *Quando*: O verso é arpejo limpo com chorus — o inverso exato deste patch.
   *Dica*: É o patch DRY01AR em um stomp só.

2. **Solo** — **EQ → ON**
   *Quando*: Sobressalente: um EQ realce para o solo, se tocar no mesmo patch.
...
**O que este patch é**: Refrão: wall of sound de DS-1 — _do quiet-grunge para o estouro absoluto_.

Verso em arpejos limpos com chorus; refrão é parede de DS-1; solo no meio.

**Teste recomendado**: refrão ("one baby to another says I'm lucky…") · **Drum**: grunge 122 bpm

**Como saber que está certo**:

- ☑️ parede de ruído controlada
- ☑️ nada do arpejo vazando no refrão
...
|---|---|---|
| DST | La Charger | Gain: 64 · Tone: 48 · Volume: 62 |
...
**Nome no painel**: `DRY01RI` · **Slot sugerido**: **U67**

1. **GP-100 Edits (recomendado)**: conecte a GP-100 por USB → importe `<NOME>.prst` desta pasta → salve no slot **U67**.
2. **Digitar no painel** (receita na ordem dos menus):

```
**DST** `La Charger` (Gain 64 / Tone 48 / Volume 62) → **AMP** `Dark Twin` (Vol 55 / Output 62 / Bass 52 / Middle 48 / Treble 55 / Bright Off) → **NR** `Gate 1` (Thr 26) → **CAB** `DarkTW 2x12` (Level 75 / High Cut 50) → **RVB** `Room` (ajuste fino no painel — seção 3 📡) → SOBRESSALENTE **EQ** `EQ 1` (template de fábrica — ajuste por orelha ao ligar) · SOBRESSALENTE **MOD** `A-Chorus` (template de fábrica — ajuste por orelha ao ligar)
```

3. **SAVE** no slot → renomeie para `DRY01RI`.
