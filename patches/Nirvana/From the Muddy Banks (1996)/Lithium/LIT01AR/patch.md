...
1. Arpejo pop, palhetada alternada
2. Com o chorus quase imperceptível: sustentação, não efeito

> 🚀 **Comece por aqui (3 passos)**:
> 1. Ajuste a guitarra conforme a tabela acima.
> 2. Carregue o patch no slot **U77** (seção 8).
> 3. Toque *verso ("I'm so happy…")* e confira o checklist da seção 5.
...
1. **Refrão "yeah yeah"** — **DST → ON**
   *Quando*: Refrão: DST ON = acordes estourados.
   *Dica*: Arpejo volta com DST OFF.

2. **Solo** — **EQ → ON**
   *Quando*: Sobressalente para o solo curto.
...
**O que este patch é**: Verso: arpejos pop com chorus — a base da música é um colchão limpo.

Arpejos pop no verso, "yeah yeah" estourado no refrão, solo curto na ponte.

**Teste recomendado**: verso ("I'm so happy…") · **Drum**: grunge 120 bpm

**Como saber que está certo**:

- ☑️ arpejo pop redondo
- ☑️ transição para o refrão sujo
...
**Nome no painel**: `LIT01AR` · **Slot sugerido**: **U77**

1. **GP-100 Edits (recomendado)**: conecte a GP-100 por USB → importe `<NOME>.prst` desta pasta → salve no slot **U77**.
...
3. **SAVE** no slot → renomeie para `LIT01AR`.
