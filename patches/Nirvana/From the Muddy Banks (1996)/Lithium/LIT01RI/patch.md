...
1. Acordes cheios, palhetada total
2. Leve embolamento é aceitável: é grunge

> 🚀 **Comece por aqui (3 passos)**:
> 1. Ajuste a guitarra conforme a tabela acima.
> 2. Carregue o patch no slot **U78** (seção 8).
> 3. Toque *refrão* e confira o checklist da seção 5.
...
|---|---|
| lama nos graves | Tone 8 |
| sem corpo | seletor 3 · Gain +2 |
...
1. **Verso pop** — **DST → OFF** + **MOD → ON**
   *Quando*: O verso é arpejo com chorus — este patch em um stomp.
   *Dica*: É o LIT01AR sem trocar de patch.

2. **Solo** — **EQ → ON**
   *Quando*: Sobressalente: realce de médios.
...
**O que este patch é**: Refrão: "yeah, yeah" com acordes estourados.

Arpejos pop no verso, "yeah yeah" estourado no refrão, solo curto na ponte.

**Teste recomendado**: refrão · **Drum**: grunge 120 bpm

**Como saber que está certo**:

- ☑️ acordes estourados com "yeah, yeah"
- ☑️ energia do refrão inteiro
...
**Nome no painel**: `LIT01RI` · **Slot sugerido**: **U78**

1. **GP-100 Edits (recomendado)**: conecte a GP-100 por USB → importe `<NOME>.prst` desta pasta → salve no slot **U78**.
...
3. **SAVE** no slot → renomeie para `LIT01RI`.
