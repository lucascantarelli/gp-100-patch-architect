...
1. Frases curtas, poucas notas
2. Bend no final para fechar

> 🚀 **Comece por aqui (3 passos)**:
> 1. Ajuste a guitarra conforme a tabela acima.
> 2. Carregue o patch no slot **U79** (seção 8).
> 3. Toque *solo (após 2º refrão)* e confira o checklist da seção 5.
...
1. **Solo cortando** — **EQ → ON**
   *Quando*: Frases curtas com presença no seletor da ponte.
...
**O que este patch é**: Solo curto e direto: distorção com o seletor na ponte.

Arpejos pop no verso, "yeah yeah" estourado no refrão, solo curto na ponte.

**Teste recomendado**: solo (após 2º refrão) · **Drum**: grunge 120 bpm

**Como saber que está certo**:

- ☑️ lead enxuto e direto
- ☑️ respirando no espaço do refrão
...
**Nome no painel**: `LIT01SO` · **Slot sugerido**: **U79**

1. **GP-100 Edits (recomendado)**: conecte a GP-100 por USB → importe `<NOME>.prst` desta pasta → salve no slot **U79**.
...
3. **SAVE** no slot → renomeie para `LIT01SO`.
