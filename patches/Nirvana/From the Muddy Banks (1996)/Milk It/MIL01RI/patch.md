...
1. Verso: acordes secos e espaçados com DST OFF (stomp)
2. Refrão: DST ON e a parede desce

> 🚀 **Comece por aqui (3 passos)**:
> 1. Ajuste a guitarra conforme a tabela acima.
> 2. Carregue o patch no slot **U86** (seção 8).
...
|---|---|
| refrão fraco | Gain +2 |
| verso sujo demais | Vol da guitarra 8 |
...
1. **Verso seco** — **DST → OFF**
   *Quando*: Pausa seca do verso — DST OFF e o patch respira.
   *Dica*: A parede volta com DST ON.

2. **Solo** — **EQ → ON**
   *Quando*: Sobressalente: realce para o final.
...
**O que este patch é**: A montanha-russa de In Utero: pausa seca → parede de DS-1 — _stomp DST é a música_.

Pausa seca → parede de DS-1: a montanha-russa quiet-loud (Seattle 1994).

**Teste recomendado**: verso + refrão · **Drum**: grunge 110 bpm

**Como saber que está certo**:

- ☑️ silêncio e peso alternando
- ☑️ punch do corte seco
...
|---|---|---|
| DST | La Charger | Gain: 70 · Tone: 42 · Volume: 62 |
...
**Nome no painel**: `MIL01RI` · **Slot sugerido**: **U86**

1. **GP-100 Edits (recomendado)**: conecte a GP-100 por USB → importe `<NOME>.prst` desta pasta → salve no slot **U86**.
2. **Digitar no painel** (receita na ordem dos menus):

```
**DST** `La Charger` (Gain 70 / Tone 42 / Volume 62) → **AMP** `Dark Twin` (Vol 55 / Output 62 / Bass 52 / Middle 48 / Treble 55 / Bright Off) → **NR** `Gate 1` (Thr 26) → **CAB** `DarkTW 2x12` (Level 75 / High Cut 50) → **RVB** `Room` (ajuste fino no painel — seção 3 📡) → SOBRESSALENTE **EQ** `EQ 1` (template de fábrica — ajuste por orelha ao ligar)
```

3. **SAVE** no slot → renomeie para `MIL01RI`.
