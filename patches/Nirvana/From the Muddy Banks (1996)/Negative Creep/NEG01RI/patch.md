...
1. Drop D: afine a 6ª em D e riff cromático na corda grave
2. Palm mute no verso, aberto no refrão

> 🚀 **Comece por aqui (3 passos)**:
> 1. Ajuste a guitarra conforme a tabela acima.
> 2. Carregue o patch no slot **U87** (seção 8).
...
1. **Solo com feedback** — **EQ → ON**
   *Quando*: Sobressalente: realce para o grito de guitarra.
...
**O que este patch é**: Drop D cromático e doente: o riff mais pesado do álbum.

Drop D cromático e doente; solo é grito de guitarra com feedback (Paramount 1991).

**Teste recomendado**: riff principal · **Drum**: grunge 110 bpm

**Como saber que está certo**:

- ☑️ riff cromático pesado com drop D
- ☑️ sujeira controlada nos graves
...
|---|---|---|
| DST | La Charger | Gain: 70 · Tone: 40 · Volume: 62 |
...
**Nome no painel**: `NEG01RI` · **Slot sugerido**: **U87**

1. **GP-100 Edits (recomendado)**: conecte a GP-100 por USB → importe `<NOME>.prst` desta pasta → salve no slot **U87**.
2. **Digitar no painel** (receita na ordem dos menus):

```
**DST** `La Charger` (Gain 70 / Tone 40 / Volume 62) → **AMP** `Dark Twin` (Vol 55 / Output 62 / Bass 52 / Middle 48 / Treble 55 / Bright Off) → **NR** `Gate 1` (Thr 26) → **CAB** `DarkTW 2x12` (Level 75 / High Cut 50) → **RVB** `Room` (ajuste fino no painel — seção 3 📡) → SOBRESSALENTE **EQ** `EQ 1` (template de fábrica — ajuste por orelha ao ligar)
```

3. **SAVE** no slot → renomeie para `NEG01RI`.
