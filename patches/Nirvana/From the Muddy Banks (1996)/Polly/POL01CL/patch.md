...
1. Verso: acordes em palm mute, seco
2. Refrão: palhetada total e para baixo — aqui o Tone da guitarra sobe para 9

> 🚀 **Comece por aqui (3 passos)**:
> 1. Ajuste a guitarra conforme a tabela acima.
> 2. Carregue o patch no slot **U89** (seção 8).
> 3. Toque *verso + refrão (elétrica 1989)* e confira o checklist da seção 5.
...
|---|---|
| refrão fraco | Vol 10 + Tone 9 |
| verso brilhante demais | Tone 5 |
...
1. **Refrão elétrico** — **DST → ON**
   *Quando*: Refrão: a elétrica de 1989 explode — DST ON.
   *Dica*: Verso volta em palm mute seco.

2. **Verso com mordida** — **EQ → ON**
   *Quando*: Sobressalente: leve realce se o verso estiver tímido.
...
**O que este patch é**: A elétrica RARA (1989): verso em palm mute limpo e refrão que explode — _o antônimo da Unplugged_.

A elétrica rara de 1989: palm mute seco no verso, explosão no refrão — antônimo da Unplugged.

**Teste recomendado**: verso + refrão (elétrica 1989) · **Drum**: 80 bpm

**Como saber que está certo**:

- ☑️ palm mute seco no verso
- ☑️ explosão de refrão (stomp DST)
...
|---|---|---|---|---|---|---|---|---|
| ~~⚪~~ PRE | ~~⚪~~ DST | **🔴** AMP · Dark Twin | ~~⚪~~ NR | **🔴** CAB · DarkTW 2x12 | ~~⚪~~ EQ | ~~⚪~~ MOD | ~~⚪~~ DLY | **🔴** RVB · Room |
...
**Nome no painel**: `POL01CL` · **Slot sugerido**: **U89**

1. **GP-100 Edits (recomendado)**: conecte a GP-100 por USB → importe `<NOME>.prst` desta pasta → salve no slot **U89**.
2. **Digitar no painel** (receita na ordem dos menus):

```
**AMP** `Dark Twin` (Vol 55 / Output 62 / Bass 52 / Middle 48 / Treble 55 / Bright Off) → **CAB** `DarkTW 2x12` (Level 75 / High Cut 50) → **RVB** `Room` (ajuste fino no painel — seção 3 📡) → SOBRESSALENTE **DST** `Blues OD` (template de fábrica — ajuste por orelha ao ligar) · SOBRESSALENTE **EQ** `EQ 1` (template de fábrica — ajuste por orelha ao ligar)
```

3. **SAVE** no slot → renomeie para `POL01CL`.
