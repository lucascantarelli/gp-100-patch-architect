...
1. Bends de tom inteiro com vibrato pesado
2. Use o captador da ponte: o solo precisa morder
...
|---|---|
| some no mix | La Charger Level +4 |
| agudo gelando | Tone da guitarra 8 |
...
1. **Solo cortando** — **EQ → ON**
   *Quando*: O solo é quase um outro riff — realce para morder.
...
**O que este patch é**: Solo central: mesma distorção, seletor na ponte — _o solo é quase um outro riff_.

O riff mais copyado do grunge: três acordes, ganho no teto, solo central.

**Teste recomendado**: solo central · **Drum**: punk rápido

**Como saber que está certo**:

- ☑️ lead cortando o embalo do riff
- ☑️ guia com a mão, não com o pedal
...
|---|---|---|
| DST | La Charger | Gain: 66 · Tone: 45 · Volume: 64 |
...
**Nome no painel**: `SCH01SO` · **Slot sugerido**: **U65**
...
```
**DST** `La Charger` (Gain 66 / Tone 45 / Volume 64) → **AMP** `Dark Twin` (Vol 55 / Output 62 / Bass 52 / Middle 48 / Treble 55 / Bright Off) → **NR** `Gate 1` (Thr 26) → **CAB** `DarkTW 2x12` (Level 75 / High Cut 50) → **RVB** `Room` (ajuste fino no painel — seção 3 📡) → SOBRESSALENTE **EQ** `EQ 1` (Mid 3 / Level 56)
```

3. **SAVE** no slot → renomeie para `SCH01SO`.
