...
> **⚡ Ajuste a Strat antes de tocar: SELETOR 4 · Volume 8 · Tone 7**
...
1. Acordes longos com swells de volume
2. A vibe dá o ar psicodélico do sample

> 🚀 **Comece por aqui (3 passos)**:
> 1. Ajuste a guitarra conforme a tabela acima.
> 2. Carregue o patch no slot **U81** (seção 8).
> 3. Toque *intro (antes do "grandma")* e confira o checklist da seção 5.
...
1. **Riff principal** — **DST → ON** + **MOD → OFF**
   *Quando*: Entrada do riff: chorus sai, distorção entra.
   *Dica*: Volte à textura no verso seguinte.

2. **Verso sem vibe** — **MOD → OFF**
   *Quando*: Verso "grandma" em palm mute, sem o colchão da vibe.
...
**O que este patch é**: A intro do coro infantil (sample no show): colchão limpo com vibe.

"Grandma take me home": palm mute no verso, DS-1 no refrão; intro de coro com vibe (era In Utero).

**Teste recomendado**: intro (antes do "grandma") · **Drum**: grunge 140 bpm

**Como saber que está certo**:

- ☑️ colchão ondulante com vibe
- ☑️ entrada do riff com contraste total
...
|---|---|---|---|---|---|---|---|---|
| ~~⚪~~ PRE | ~~⚪~~ DST | **🔴** AMP · Dark Twin | ~~⚪~~ NR | **🔴** CAB · DarkTW 2x12 | ~~⚪~~ EQ | **🔴** MOD · Vibe | ~~⚪~~ DLY | **🔴** RVB · Room |
...
| MOD | Vibe | Intensidade: 45 · Velocidade: 0.5 · Sync: Off |
...
**Nome no painel**: `SLV01AR` · **Slot sugerido**: **U81**

1. **GP-100 Edits (recomendado)**: conecte a GP-100 por USB → importe `<NOME>.prst` desta pasta → salve no slot **U81**.
2. **Digitar no painel** (receita na ordem dos menus):

```
**AMP** `Dark Twin` (Vol 55 / Output 62 / Bass 52 / Middle 48 / Treble 55 / Bright Off) → **CAB** `DarkTW 2x12` (Level 75 / High Cut 50) → **MOD** `Vibe` (Intensidade 45 / Velocidade 0.5 / Sync Off) → **RVB** `Room` (ajuste fino no painel — seção 3 📡) → SOBRESSALENTE **DST** `Blues OD` (template de fábrica — ajuste por orelha ao ligar)
```

3. **SAVE** no slot → renomeie para `SLV01AR`.
