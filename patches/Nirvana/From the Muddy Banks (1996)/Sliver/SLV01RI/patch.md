...
> 💡 **"Grandma take me home": palm mute no verso, DS-1 no refrão — _o stomp DST troca a seção_**
...
1. Verso: acordes secos em palm mute com o DST DESLIGADO (stomp)
2. Refrão: ligue o DST e afogue tudo

> 🚀 **Comece por aqui (3 passos)**:
> 1. Ajuste a guitarra conforme a tabela acima.
> 2. Carregue o patch no slot **U80** (seção 8).
...
1. **Verso seco** — **DST → OFF**
   *Quando*: "Grandma take me home" em palm mute SECO — DST OFF.
   *Dica*: Refrão volta com DST ON.

2. **Intro do coro** — **MOD → ON**
   *Quando*: A intro é colchão com vibe (patch SLV01AR) — aqui é um stomp.
...
**O que este patch é**: "Grandma take me home": palm mute no verso, DS-1 no refrão — _o stomp DST troca a seção_.

"Grandma take me home": palm mute no verso, DS-1 no refrão; intro de coro com vibe (era In Utero).

**Teste recomendado**: verso + refrão · **Drum**: grunge 140 bpm

**Como saber que está certo**:

- ☑️ contraste claro entre verso seco e refrão sujo
- ☑️ punch do palm mute
...
**Nome no painel**: `SLV01RI` · **Slot sugerido**: **U80**

1. **GP-100 Edits (recomendado)**: conecte a GP-100 por USB → importe `<NOME>.prst` desta pasta → salve no slot **U80**.
