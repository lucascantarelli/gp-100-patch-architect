# 🎸 SLT01SO
### Smells Like Teen Spirit — Solo Gaguejado
...
1. Toque a melodia do vocal em blocos de duas notas
2. Repita a frase com pequenas variações (a gagueira é proposital)

> 🚀 **Comece por aqui (3 passos)**:
> 1. Ajuste a guitarra conforme a tabela acima.
> 2. Carregue o patch no slot **U74** (seção 8).
> 3. Toque *solo (após o 2º refrão)* e confira o checklist da seção 5.
...
1. **Solo cortando** — **EQ → ON**
   *Quando*: A melodia gaguejada pede médios à frente.
...
**O que este patch é**: Solo: a melodia do vocal gaguejada na distorção — praticável e icônico.

O hino: verso limpo com chorus (o erro clássico é tocar sujo), refrão de DS-1, solo gaguejado.

**Teste recomendado**: solo (após o 2º refrão) · **Drum**: grunge 117 bpm

**Como saber que está certo**:

- ☑️ melodia do vocal reconhecível na distorção
- ☑️ blocos de duas notas
...
**Nome no painel**: `SLT01SO` · **Slot sugerido**: **U74**

1. **GP-100 Edits (recomendado)**: conecte a GP-100 por USB → importe `<NOME>.prst` desta pasta → salve no slot **U74**.
2. **Digitar no painel** (receita na ordem dos menus):

```
**DST** `La Charger` (Gain 66 / Tone 50 / Volume 64) → **AMP** `Dark Twin` (Vol 55 / Output 62 / Bass 52 / Middle 48 / Treble 55 / Bright Off) → **NR** `Gate 1` (Thr 26) → **CAB** `DarkTW 2x12` (Level 75 / High Cut 50) → **RVB** `Room` (ajuste fino no painel — seção 3 📡) → SOBRESSALENTE **EQ** `EQ 1` (Mid 3 / Level 56)
```

3. **SAVE** no slot → renomeie para `SLT01SO`.
