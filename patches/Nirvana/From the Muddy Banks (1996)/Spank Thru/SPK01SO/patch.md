...
1. Jam: toque o que a base pede, com bends e vibrato
2. A estrutura é solta: escute a base e responda

> 🚀 **Comece por aqui (3 passos)**:
> 1. Ajuste a guitarra conforme a tabela acima.
> 2. Carregue o patch no slot **U82** (seção 8).
> 3. Toque *música inteira (instrumental)* e confira o checklist da seção 5.
...
|---|---|
| falta coesão | toque menos notas |
| gelando | Tone 8 |
...
1. **Base punk** — **DST → OFF** + **MOD → ON**
   *Quando*: Para a base da jam, desligue o lead: chorus limpo.
   *Dica*: Solo volta com DST ON.
...
**O que este patch é**: A única versão lançada do show de Roma: jam instrumental do início da banda.

Jam instrumental do show de Roma (nov/1991) — a única versão lançada dessa noite.

**Teste recomendado**: música inteira (instrumental) · **Drum**: jam 130 bpm

**Como saber que está certo**:

- ☑️ jam instrumental coesa
- ☑️ frases respondendo à base
...
**Nome no painel**: `SPK01SO` · **Slot sugerido**: **U82**

1. **GP-100 Edits (recomendado)**: conecte a GP-100 por USB → importe `<NOME>.prst` desta pasta → salve no slot **U82**.
2. **Digitar no painel** (receita na ordem dos menus):

```
**DST** `La Charger` (Gain 66 / Tone 50 / Volume 64) → **AMP** `Dark Twin` (Vol 55 / Output 62 / Bass 52 / Middle 48 / Treble 55 / Bright Off) → **NR** `Gate 1` (Thr 26) → **CAB** `DarkTW 2x12` (Level 75 / High Cut 50) → **RVB** `Room` (ajuste fino no painel — seção 3 📡) → SOBRESSALENTE **MOD** `A-Chorus` (template de fábrica — ajuste por orelha ao ligar)
```

3. **SAVE** no slot → renomeie para `SPK01SO`.
