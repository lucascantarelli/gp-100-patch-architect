# 🧱 ABIETWBA
### Another Brick in the Wall, Part 2 — Base
...
1. Riff com palhetada firme
2. Mute com a mão direita no groove

> 🚀 **Comece por aqui (3 passos)**:
> 1. Ajuste a guitarra conforme a tabela acima.
> 2. Carregue o patch no slot **U40** (seção 8).
> 3. Toque *riff e refrões* e confira o checklist da seção 5.
...
1. **Solo (eco)** — **DLY → ON**
   *Quando*: Chegou o solo: ligue o DLY e o patch vira o solo do Muro.
...
**O que este patch é**: Base do muro: crunch seco e groove disco — o refrão coral pede firmeza.

O solo do 'Muro': um dos mais icônicos — TD + delay, frases esparsas com propósito.

**Teste recomendado**: riff e refrões · **Drum**: Rock/groove 123 BPM

**Como saber que está certo**:

- ☑️ crunch seco no groove
...
|---|---|---|
| Solo icônico: TD + delay, frases esparsas com propósito | alta | Pulse (1995) · gilmourish.com |
...
|---|---|---|
| PRE | Saturate | Gain: 35 · Mix: 55 · Output: 60 |
...
**Nome no painel**: `ABIETWBA` · **Slot sugerido**: **U40**

1. **GP-100 Edits (recomendado)**: conecte a GP-100 por USB → importe `<NOME>.prst` desta pasta → salve no slot **U40**.
2. **Digitar no painel** (receita na ordem dos menus):

```
**PRE** `Saturate` (Gain 35 / Mix 55 / Output 60) → **AMP** `Flagman` (Gain 44 / PRSE 55 / Master 62 / Bass 50 / Middle 55 / Treble 58) → **NR** `Gate 1` (Thr 26) → **CAB** `UK-LD 4x12` (Level 78 / High Cut 62) → **RVB** `Plate` (ajuste fino no painel — seção 3 📡) → SOBRESSALENTE **DLY** `Sweet` (Mix 15 / Time 430 ms / Fdbk 25)
```

3. **SAVE** no slot → renomeie para `ABIETWBA`.
