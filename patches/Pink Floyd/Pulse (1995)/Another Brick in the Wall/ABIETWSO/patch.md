...
2. Deixe o delay responder entre as frases

> 🚀 **Comece por aqui (3 passos)**:
> 1. Ajuste a guitarra conforme a tabela acima.
> 2. Carregue o patch no slot **U41** (seção 8).
...
1. **Base seca** — **DLY → OFF**
   *Quando*: Groove do riff sem eco (só o solo usa).
...
**O que este patch é**: O solo do muro: TD quente + delay — frases esparsas, cada nota com propósito.

O solo do 'Muro': um dos mais icônicos — TD + delay, frases esparsas com propósito.

**Teste recomendado**: solo central · **Drum**: Rock/groove 123 BPM

**Como saber que está certo**:

- ☑️ frases esparsas e melódicas
...
**Nome no painel**: `ABIETWSO` · **Slot sugerido**: **U41**

1. **GP-100 Edits (recomendado)**: conecte a GP-100 por USB → importe `<NOME>.prst` desta pasta → salve no slot **U41**.
...
3. **SAVE** no slot → renomeie para `ABIETWSO`.
