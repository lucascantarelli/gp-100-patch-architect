...
1. Frases longas com vibrato
2. Deixe o delay criar os padrões

> 🚀 **Comece por aqui (3 passos)**:
> 1. Ajuste a guitarra conforme a tabela acima.
> 2. Carregue o patch no slot **U54** (seção 8).
> 3. Toque *jam improvisada* e confira o checklist da seção 5.
...
**O que este patch é**: Jam espacial: fuzz + delay longo — a guitarra conversa com os sintetizadores.

Jam do DSOTM: guitarra espacial com fuzz e delays, diálogo com os sintetizadores.

**Teste recomendado**: jam improvisada · **Drum**: Instrumental espacial

**Como saber que está certo**:

- ☑️ fuzz sustentado
- ☑️ delay criando padrões
...
|---|---|---|
| Fuzz + delays espaciais; jam improvisada | alta | Pulse (1995) · gilmourish.com |
...
**Nome no painel**: `CLR01JM` · **Slot sugerido**: **U54**

1. **GP-100 Edits (recomendado)**: conecte a GP-100 por USB → importe `<NOME>.prst` desta pasta → salve no slot **U54**.
2. **Digitar no painel** (receita na ordem dos menus):

```
**DST** `Red Haze` (p0 55 / p1 50 / p2 55) → **AMP** `Flagman` (p0 40 / p1 50 / p2 58 / p3 50 / p4 52 / p5 56) → **NR** `Gate 1` (Thr 30) → **CAB** `UK-LD 4x12` (p0 78 / p1 62) → **DLY** `Sweet` (Fdbk 35 / Delay ms 570 / High Cut 28) → **RVB** `Hall` (ajuste fino no painel — seção 3 📡)
```
