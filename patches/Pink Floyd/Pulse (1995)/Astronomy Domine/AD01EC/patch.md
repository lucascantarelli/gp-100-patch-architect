...
> **⚡ Ajuste a Strat antes de tocar: SELETOR 2 · Volume 9 · Tone 8-9**
...
1. Ataques curtos e secos — o delay cria o padrão rítmico
2. Experimente tocar ENTRE as batidas do eco

> 🚀 **Comece por aqui (3 passos)**:
> 1. Ajuste a guitarra conforme a tabela acima.
> 2. Carregue o patch no slot **U28** (seção 8).
> 3. Toque *riffs de eco entre os acordes* e confira o checklist da seção 5.
...
|---|---|
| eco embolando | DLY Fdbk -8 |
| falta mordida | Knights CL Gain +5 |
...
**O que este patch é**: Base com eco curto: o repetidor faz o ritmo — limpo crocante estilo Syd.

Syd Barrett (1967) revisitado ao vivo: guitarra de eco metálico — o delay é instrumento.

**Teste recomendado**: riffs de eco entre os acordes · **Drum**: Rock espacial

**Como saber que está certo**:

- ☑️ eco curto repetindo o ataque
- ☑️ crocante sem sumir na lama
...
|---|---|---|
| Versão Pulse: solo com eco curto repetido, estilo Space Echo | alta | Pulse (1995) · gilmourish.com |
...
|---|---|---|---|---|---|---|---|---|
| ~~⚪~~ PRE | ~~⚪~~ DST | **🔴** AMP · Knights CL | **🔴** NR · Gate 1 | **🔴** CAB · UK-LD 4x12 | ~~⚪~~ EQ | ~~⚪~~ MOD | **🔴** DLY · Sweet | **🔴** RVB · Room |
...
|---|---|---|
| AMP | Knights CL | Gain: 42 · Vol: 62 · Bass: 62 · Middle: 48 · Treble: 52 |
| NR | Gate 1 | Thr: 26 |
| CAB | UK-LD 4x12 | Level: 78 · High Cut: 62 |
| DLY | Sweet | Mix: 30 · Time: 180 ms · Fdbk: 22 |
...
**Nome no painel**: `AD01EC` · **Slot sugerido**: **U28**

1. **GP-100 Edits (recomendado)**: conecte a GP-100 por USB → importe `<NOME>.prst` desta pasta → salve no slot **U28**.
2. **Digitar no painel** (receita na ordem dos menus):

```
**AMP** `Knights CL` (Gain 42 / Vol 62 / Bass 62 / Middle 48 / Treble 52) → **NR** `Gate 1` (Thr 26) → **CAB** `UK-LD 4x12` (Level 78 / High Cut 62) → **DLY** `Sweet` (Mix 30 / Time 180 ms / Fdbk 22) → **RVB** `Room` (ajuste fino no painel — seção 3 📡)
```

3. **SAVE** no slot → renomeie para `AD01EC`.
