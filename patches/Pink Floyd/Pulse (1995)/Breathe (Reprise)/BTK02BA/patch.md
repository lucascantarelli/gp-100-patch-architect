...
1. Acordes com palhetada lenta e pesada
2. Deixe o sustain completar os acordes

> 🚀 **Comece por aqui (3 passos)**:
> 1. Ajuste a guitarra conforme a tabela acima.
> 2. Carregue o patch no slot **U48** (seção 8).
> 3. Toque *acordes da reprise* e confira o checklist da seção 5.
...
**O que este patch é**: Reprise: crunch suave e escuro — a ponte entre Time e Money.

Reprise: base com crunch suave e clima mais escuro.

**Teste recomendado**: acordes da reprise · **Drum**: DSOTM 63 BPM
...
|---|---|---|
| Base com crunch suave e clima escuro | alta | Pulse (1995) · gilmourish.com |
...
**Nome no painel**: `BTK02BA` · **Slot sugerido**: **U48**

1. **GP-100 Edits (recomendado)**: conecte a GP-100 por USB → importe `<NOME>.prst` desta pasta → salve no slot **U48**.
2. **Digitar no painel** (receita na ordem dos menus):

```
**PRE** `Saturate` (p0 30 / p1 55 / p2 60) → **AMP** `Flagman` (p0 44 / p1 55 / p2 62 / p3 50 / p4 55 / p5 58) → **NR** `Gate 1` (Thr 26) → **CAB** `UK-LD 4x12` (p0 78 / p1 62) → **RVB** `Plate` (ajuste fino no painel — seção 3 📡) → SOBRESSALENTE **DLY** `Sweet` (template de fábrica — ajuste por orelha ao ligar)
```
