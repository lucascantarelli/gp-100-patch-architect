...
1. Acordes Em7/A7 com palhetada lenta
2. Deixe as cordas soar — pouco mute

> 🚀 **Comece por aqui (3 passos)**:
> 1. Ajuste a guitarra conforme a tabela acima.
> 2. Carregue o patch no slot **U44** (seção 8).
> 3. Toque *acordes da intro e refrões* e confira o checklist da seção 5.
...
1. **Solo espacial** — **DLY → ON**
   *Quando*: Sobressalente para frases entre os acordes.
...
**O que este patch é**: Base spacey: chorus sutil e reverb amplo — o respiro do DSOTM.

DSOTM: acordes com chorus suave e reverb amplo — a base do respiro.

**Teste recomendado**: acordes da intro e refrões · **Drum**: DSOTM 63 BPM
...
|---|---|---|
| Base com chorus sutil + reverb; clima spacey | alta | Pulse (1995) · gilmourish.com |
...
**Nome no painel**: `BRT01BA` · **Slot sugerido**: **U44**

1. **GP-100 Edits (recomendado)**: conecte a GP-100 por USB → importe `<NOME>.prst` desta pasta → salve no slot **U44**.
2. **Digitar no painel** (receita na ordem dos menus):

```
**AMP** `Knights CL` (p0 38 / p1 62 / p2 62 / p3 48 / p4 52 / p5 58) → **CAB** `UK-LD 4x12` (p0 78 / p1 62) → **MOD** `A-Chorus` (Rate 25 / Depth 0.5 / Mix 25) → **RVB** `Hall` (ajuste fino no painel — seção 3 📡) → SOBRESSALENTE **DLY** `Sweet` (template de fábrica — ajuste por orelha ao ligar)
```
