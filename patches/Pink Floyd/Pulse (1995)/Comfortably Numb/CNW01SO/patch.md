...
1. Bend de 1 tom na subida
2. Frases com espaço — o delay responde

> 🚀 **Comece por aqui (3 passos)**:
> 1. Ajuste a guitarra conforme a tabela acima.
> 2. Carregue o patch no slot **U58** (seção 8).
> 3. Toque *1º solo* e confira o checklist da seção 5.
...
1. **Acordes do refrão** — **DLY → OFF**
   *Quando*: Os acordes da música pedem seco; o eco é do solo.
...
**O que este patch é**: 1º solo (após os versos): Tube Driver quente + delay — a subida que atravessa o muro.

O solo dos solos: duas subidas épicas com TD + delay — o hino do rock.

**Teste recomendado**: 1º solo · **Drum**: Épico 134 BPM

**Como saber que está certo**:

- ☑️ frases ascendentes com sustain
- ☑️ eco na duração da nota
...
**Nome no painel**: `CNW01SO` · **Slot sugerido**: **U58**

1. **GP-100 Edits (recomendado)**: conecte a GP-100 por USB → importe `<NOME>.prst` desta pasta → salve no slot **U58**.
...
3. **SAVE** no slot → renomeie para `CNW01SO`.
