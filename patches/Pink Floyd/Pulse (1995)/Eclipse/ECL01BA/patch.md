...
1. Acordes longos e maciços
2. Sincronize com o coro final

> 🚀 **Comece por aqui (3 passos)**:
> 1. Ajuste a guitarra conforme a tabela acima.
> 2. Carregue o patch no slot **U56** (seção 8).
> 3. Toque *acordes finais* e confira o checklist da seção 5.
...
**O que este patch é**: Clímax: acordes maciços com crunch e reverb amplo — o fecho do DSOTM.

Final do DSOTM: acordes maciços com crunch e reverb — o clímax do álbum.

**Teste recomendado**: acordes finais · **Drum**: DSOTM 84 BPM
...
|---|---|---|
| Acordes maciços com crunch e reverb | alta | Pulse (1995) · gilmourish.com |
...
|---|---|---|---|---|---|---|---|---|
| **🔴** PRE · Saturate | ~~⚪~~ DST | **🔴** AMP · Flagman | **🔴** NR · Gate 1 | **🔴** CAB · UK-LD 4x12 | ~~⚪~~ EQ | ~~⚪~~ MOD | ~~⚪~~ DLY | **🔴** RVB · Hall |
...
**Nome no painel**: `ECL01BA` · **Slot sugerido**: **U56**

1. **GP-100 Edits (recomendado)**: conecte a GP-100 por USB → importe `<NOME>.prst` desta pasta → salve no slot **U56**.
2. **Digitar no painel** (receita na ordem dos menus):

```
**PRE** `Saturate` (p0 36 / p1 55 / p2 62) → **AMP** `Flagman` (p0 44 / p1 55 / p2 62 / p3 50 / p4 55 / p5 58) → **NR** `Gate 1` (Thr 26) → **CAB** `UK-LD 4x12` (p0 78 / p1 62) → **RVB** `Hall` (ajuste fino no painel — seção 3 📡)
```
