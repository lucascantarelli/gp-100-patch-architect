...
> **⚡ Ajuste a Strat antes de tocar: SELETOR 2 · Volume 10 · Tone 8**
...
1. Notas longas com vibrato largo
2. Frases esparsas: o fuzz sustenta tudo

> 🚀 **Comece por aqui (3 passos)**:
> 1. Ajuste a guitarra conforme a tabela acima.
> 2. Carregue o patch no slot **U35** (seção 8).
> 3. Toque *2º solo (clímax)* e confira o checklist da seção 5.
...
1. **Verso sem fuzz** — **DST → OFF**
   *Quando*: O Big Muff sai e sobra o crunch do Flagman — um patch para a música toda.
   *Dica*: Religue o DST no solo.
...
**O que este patch é**: Solo psicotélico: Big Muff saturado — o uivo de fuzz do clímax.

A épica de 1994: solo principal com Tube Driver + delay, e solo psicotélico com Big Muff.

**Teste recomendado**: 2º solo (clímax) · **Drum**: Rock épico

**Como saber que está certo**:

- ☑️ fuzz sustentado sem virar ruído
- ☑️ notas definidas dentro do fuzz
...
|---|---|---|
| Solo principal TD+delay; 2º solo Big Muff Civil War | alta | Pulse (1995) · gilmourish.com |
...
|---|---|---|---|---|---|---|---|---|
| ~~⚪~~ PRE | **🔴** DST · Red Haze | **🔴** AMP · Flagman | **🔴** NR · Gate 1 | **🔴** CAB · UK-LD 4x12 | ~~⚪~~ EQ | ~~⚪~~ MOD | **🔴** DLY · Sweet | **🔴** RVB · Hall |
...
**Nome no painel**: `KTB01FZ` · **Slot sugerido**: **U35**

1. **GP-100 Edits (recomendado)**: conecte a GP-100 por USB → importe `<NOME>.prst` desta pasta → salve no slot **U35**.
2. **Digitar no painel** (receita na ordem dos menus):

```
**DST** `Red Haze` (p0 62 / p1 45 / p2 55) → **AMP** `Flagman` (Gain 40 / PRSE 50 / Master 58 / Bass 50 / Middle 52 / Treble 56) → **NR** `Gate 1` (Thr 30) → **CAB** `UK-LD 4x12` (Level 78 / High Cut 62) → **DLY** `Sweet` (Fdbk 12 / Delay ms 570 / High Cut 22) → **RVB** `Hall` (ajuste fino no painel — seção 3 📡)
```

3. **SAVE** no slot → renomeie para `KTB01FZ`.
