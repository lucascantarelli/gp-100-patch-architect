...
1. Bends de 1 tom com sustain longo
2. Frase com espaço: o delay responde

> 🚀 **Comece por aqui (3 passos)**:
> 1. Ajuste a guitarra conforme a tabela acima.
> 2. Carregue o patch no slot **U34** (seção 8).
...
1. **Refrão seco** — **DLY → OFF**
   *Quando*: Acordes do refrão sem eco (o canto responde).
...
**O que este patch é**: Solo principal: Tube Driver quente + delay da nota — a voz que responde Stephen Hawking.

A épica de 1994: solo principal com Tube Driver + delay, e solo psicotélico com Big Muff.

**Teste recomendado**: solo após o 2º refrão · **Drum**: Rock épico
...
**Nome no painel**: `KTB01SO` · **Slot sugerido**: **U34**

1. **GP-100 Edits (recomendado)**: conecte a GP-100 por USB → importe `<NOME>.prst` desta pasta → salve no slot **U34**.
2. **Digitar no painel** (receita na ordem dos menus):

```
**PRE** `Saturate` (Gain 50 / Mix 60 / Output 62) → **AMP** `Flagman` (Gain 50 / PRSE 58 / Master 64 / Bass 50 / Middle 55 / Treble 58) → **NR** `Gate 1` (Thr 30) → **CAB** `UK-LD 4x12` (Level 78 / High Cut 62) → **DLY** `Sweet` (Mix 20 / Time 570 ms / Fdbk 30) → **RVB** `Hall` (ajuste fino no painel — seção 3 📡)
```

3. **SAVE** no slot → renomeie para `KTB01SO`.
