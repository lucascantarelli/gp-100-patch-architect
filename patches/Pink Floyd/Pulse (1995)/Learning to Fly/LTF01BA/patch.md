...
1. Acordes abertos (add9/sus2) com palhetada lenta
2. Deixe o chorus fazer o movimento

> 🚀 **Comece por aqui (3 passos)**:
> 1. Ajuste a guitarra conforme a tabela acima.
> 2. Carregue o patch no slot **U31** (seção 8).
> 3. Toque *acordes abertos da intro e refrão* e confira o checklist da seção 5.
...
|---|---|
| chorus forte demais | MOD Mix -10 |
| seco | RVB decay +5 |
...
1. **Solo espacial** — **DLY → ON**
   *Quando*: O solo da música vive do eco — ligue o sobressalente.
...
**O que este patch é**: Base aérea: limpo com chorus lento — o som de nuvem da intro.

Anos 80 etéreo: base limpa com chorus, arpejos de eco e solo aéreo.

**Teste recomendado**: acordes abertos da intro e refrão · **Drum**: Rock 104 BPM

**Como saber que está certo**:

- ☑️ chorus amplo sem 'aquário'
- ☑️ clareza nos acordes abertos
...
|---|---|---|---|---|---|---|---|---|
| ~~⚪~~ PRE | ~~⚪~~ DST | **🔴** AMP · Knights CL | ~~⚪~~ NR | **🔴** CAB · UK-LD 4x12 | ~~⚪~~ EQ | **🔴** MOD · A-Chorus | ~~⚪~~ DLY | **🔴** RVB · Hall |
...
**Nome no painel**: `LTF01BA` · **Slot sugerido**: **U31**

1. **GP-100 Edits (recomendado)**: conecte a GP-100 por USB → importe `<NOME>.prst` desta pasta → salve no slot **U31**.
2. **Digitar no painel** (receita na ordem dos menus):

```
**AMP** `Knights CL` (Gain 38 / Vol 62 / Bass 62 / Middle 48 / Treble 52 / p5 58) → **CAB** `UK-LD 4x12` (Level 78 / High Cut 62) → **MOD** `A-Chorus` (Rate 28 / Depth 0.5 / Mix 28) → **RVB** `Hall` (ajuste fino no painel — seção 3 📡) → SOBRESSALENTE **DLY** `Sweet` (Fdbk 20 / Delay ms 570 / High Cut 30)
```

3. **SAVE** no slot → renomeie para `LTF01BA`.
