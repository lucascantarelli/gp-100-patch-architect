...
1. Toque UMA frase e espere o eco responder
2. Bends lentos com vibrato largo

> 🚀 **Comece por aqui (3 passos)**:
> 1. Ajuste a guitarra conforme a tabela acima.
> 2. Carregue o patch no slot **U32** (seção 8).
> 3. Toque *solo central* e confira o checklist da seção 5.
...
|---|---|
| eco alto demais | DLY Mix -8 |
| solo afogado | RVB mix -5 |
...
1. **Base sem eco** — **DLY → OFF**
   *Quando*: Acompanhamento limpo — o chorus segue, o eco sai.
...
**O que este patch é**: Solo aéreo com delay: frases curtas que o eco responde — diálogo com o delay.

Anos 80 etéreo: base limpa com chorus, arpejos de eco e solo aéreo.

**Teste recomendado**: solo central · **Drum**: Rock 104 BPM

**Como saber que está certo**:

- ☑️ frases respondidas pelo eco
- ☑️ notas flutuando no reverb
...
|---|---|---|
| Base limpa com chorus + delay; solo com o mesmo pad | alta | Pulse (1995) · gilmourish.com |
...
|---|---|---|---|---|---|---|---|---|
| ~~⚪~~ PRE | ~~⚪~~ DST | **🔴** AMP · Knights CL | ~~⚪~~ NR | **🔴** CAB · UK-LD 4x12 | ~~⚪~~ EQ | **🔴** MOD · A-Chorus | **🔴** DLY · Sweet | **🔴** RVB · Hall |
...
**Nome no painel**: `LTF01SO` · **Slot sugerido**: **U32**

1. **GP-100 Edits (recomendado)**: conecte a GP-100 por USB → importe `<NOME>.prst` desta pasta → salve no slot **U32**.
2. **Digitar no painel** (receita na ordem dos menus):

```
**AMP** `Knights CL` (Gain 42 / Vol 62 / Bass 62 / Middle 48 / Treble 52) → **CAB** `UK-LD 4x12` (Level 78 / High Cut 62) → **MOD** `A-Chorus` (Rate 28 / Depth 0.5 / Mix 28) → **DLY** `Sweet` (Mix 20 / Time 570 ms / Fdbk 30) → **RVB** `Hall` (ajuste fino no painel — seção 3 📡)
```

3. **SAVE** no slot → renomeie para `LTF01SO`.
