...
1. Swells com a chave de volume sobre o delay
2. Ruídos controlados com o braço na ponte

> 🚀 **Comece por aqui (3 passos)**:
> 1. Ajuste a guitarra conforme a tabela acima.
> 2. Carregue o patch no slot **U45** (seção 8).
> 3. Toque *texturas, ruídos e swells* e confira o checklist da seção 5.
...
1. **Textura crua** — **DLY → OFF**
   *Quando*: Só o reverb ambiente — o delay industrial sai.
...
**O que este patch é**: Ambiente sintetizado: delay com feedback e reverb longo — camada de textura para a faixa.

Sem guitarra na faixa (sintetizadores/efeitos) — patch de ambiente para camadas caseiras.

**Teste recomendado**: texturas, ruídos e swells · **Drum**: —

**Como saber que está certo**:

- ☑️ eco longo e industrial
...
|---|---|---|
| Sem guitarra: VCS3, sequenciador e efeitos de aeroporto | alta | Pulse (1995) · gilmourish.com |
...
|---|---|---|---|---|---|---|---|---|
| ~~⚪~~ PRE | ~~⚪~~ DST | **🔴** AMP · Flagman | ~~⚪~~ NR | **🔴** CAB · UK-LD 4x12 | ~~⚪~~ EQ | ~~⚪~~ MOD | **🔴** DLY · T-Echo | **🔴** RVB · Hall |
...
| DLY | T-Echo | Mix: 40 · Time: 590 ms · Fdbk: 30 |
...
**Nome no painel**: `ONR01AM` · **Slot sugerido**: **U45**

1. **GP-100 Edits (recomendado)**: conecte a GP-100 por USB → importe `<NOME>.prst` desta pasta → salve no slot **U45**.
2. **Digitar no painel** (receita na ordem dos menus):

```
**AMP** `Flagman` (Gain 25 / PRSE 50 / Master 55 / Bass 50 / Middle 50 / Treble 55) → **CAB** `UK-LD 4x12` (Level 78 / High Cut 62) → **DLY** `T-Echo` (Mix 40 / Time 590 ms / Fdbk 30) → **RVB** `Hall` (ajuste fino no painel — seção 3 📡)
```
