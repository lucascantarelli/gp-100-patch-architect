...
1. Riff em G5-A5 com ataques sincopados
2. Toque e solte — o delay completa o padrão
3. Mute firme com a mão direita

> 🚀 **Comece por aqui (3 passos)**:
> 1. Ajuste a guitarra conforme a tabela acima.
> 2. Carregue o patch no slot **U61** (seção 8).
> 3. Toque *riff principal (G5-A5) com espaço entre os ataques* e confira o checklist da seção 5.
...
1. **Ensaio no tempo** — **DLY → OFF**
   *Quando*: Ensaie o riff seco — o eco da parede esconde atrasos de timing. Ao vivo, religue.
...
**O que este patch é**: O riff com parede de eco: Flagman crunch + delay com feedback — o delay É o arranjo.

O fechamento: riff em G com delay fazendo a parede de som — efeito e riff são a mesma coisa.

**Teste recomendado**: riff principal (G5-A5) com espaço entre os ataques · **Drum**: Rock 148 BPM

**Como saber que está certo**:

- ☑️ eco preenchendo os espaços do riff
- ☑️ crunch firme e seco
...
|---|---|---|
| Riff com delay longo (parede de eco); solo curto | alta | Pulse (1995) · gilmourish.com |
...
|---|---|---|---|---|---|---|---|---|
| ~~⚪~~ PRE | ~~⚪~~ DST | **🔴** AMP · Flagman | **🔴** NR · Gate 1 | **🔴** CAB · UK-LD 4x12 | ~~⚪~~ EQ | ~~⚪~~ MOD | **🔴** DLY · Sweet | **🔴** RVB · Plate |
...
| DLY | Sweet | Mix: 35 · Time: 450 ms · Fdbk: 30 |
...
**Nome no painel**: `RLH01RI` · **Slot sugerido**: **U61**

1. **GP-100 Edits (recomendado)**: conecte a GP-100 por USB → importe `<NOME>.prst` desta pasta → salve no slot **U61**.
2. **Digitar no painel** (receita na ordem dos menus):

```
**AMP** `Flagman` (Gain 44 / PRSE 55 / Master 62 / Bass 50 / Middle 55 / Treble 58) → **NR** `Gate 1` (Thr 26) → **CAB** `UK-LD 4x12` (Level 78 / High Cut 62) → **DLY** `Sweet` (Mix 35 / Time 450 ms / Fdbk 30) → **RVB** `Plate` (ajuste fino no painel — seção 3 📡)
```
