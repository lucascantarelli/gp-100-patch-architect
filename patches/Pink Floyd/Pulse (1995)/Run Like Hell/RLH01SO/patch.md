...
1. Frases curtas e angulares em Mi menor
2. Use o delay como terceiro braço

> 🚀 **Comece por aqui (3 passos)**:
> 1. Ajuste a guitarra conforme a tabela acima.
> 2. Carregue o patch no slot **U62** (seção 8).
...
1. **Ritmo (sem eco)** — **DLY → OFF**
   *Quando*: Acordes do verso sem a parede de eco.
...
**O que este patch é**: Solo: Tube Driver + o mesmo delay — frases gritadas sobre a parede de eco.

O fechamento: riff em G com delay fazendo a parede de som — efeito e riff são a mesma coisa.

**Teste recomendado**: solo central · **Drum**: Rock 148 BPM

**Como saber que está certo**:

- ☑️ frases sobre a parede de eco
- ☑️ definição em andamento rápido
...
| DLY | Sweet | Mix: 30 · Time: 450 ms · Fdbk: 28 |
...
**Nome no painel**: `RLH01SO` · **Slot sugerido**: **U62**

1. **GP-100 Edits (recomendado)**: conecte a GP-100 por USB → importe `<NOME>.prst` desta pasta → salve no slot **U62**.
2. **Digitar no painel** (receita na ordem dos menus):

```
**PRE** `Saturate` (Gain 50 / Mix 60 / Output 62) → **AMP** `Flagman` (Gain 50 / PRSE 58 / Master 64 / Bass 50 / Middle 55 / Treble 58) → **NR** `Gate 1` (Thr 30) → **CAB** `UK-LD 4x12` (Level 78 / High Cut 62) → **DLY** `Sweet` (Mix 30 / Time 450 ms / Fdbk 28) → **RVB** `Hall` (ajuste fino no painel — seção 3 📡)
```

3. **SAVE** no slot → renomeie para `RLH01SO`.
