# 🔷 SOF01BA
### Shine On You Crazy Diamond — Base
...
1. Acordes abertos com palhetada suave
2. Deixe os acordes soar completos

> 🚀 **Comece por aqui (3 passos)**:
> 1. Ajuste a guitarra conforme a tabela acima.
> 2. Carregue o patch no slot **U26** (seção 8).
> 3. Toque *acordes das partes cantadas* e confira o checklist da seção 5.
...
1. **Solo (com eco)** — **DLY → ON**
   *Quando*: Frases entre os acordes: ligue e você tem o eco do solo sem trocar de patch.
...
**O que este patch é**: Base AOR: acordes abertos com brilho e espaço — o colchão das partes cantadas.

A assinatura: intro com delay a cada nota (assinatura absoluta), base AOR e solo final.

**Teste recomendado**: acordes das partes cantadas · **Drum**: AOR lento

**Como saber que está certo**:

- ☑️ acordes abertos respirando
- ☑️ brilho sem dureza
...
|---|---|---|---|---|---|---|---|---|
| ~~⚪~~ PRE | ~~⚪~~ DST | **🔴** AMP · Knights CL | ~~⚪~~ NR | **🔴** CAB · UK-LD 4x12 | ~~⚪~~ EQ | ~~⚪~~ MOD | ~~⚪~~ DLY | **🔴** RVB · Hall |
...
**Nome no painel**: `SOF01BA` · **Slot sugerido**: **U26**

1. **GP-100 Edits (recomendado)**: conecte a GP-100 por USB → importe `<NOME>.prst` desta pasta → salve no slot **U26**.
2. **Digitar no painel** (receita na ordem dos menus):

```
**AMP** `Knights CL` (Gain 38 / Vol 62 / Bass 62 / Middle 48 / Treble 52) → **CAB** `UK-LD 4x12` (Level 78 / High Cut 62) → **RVB** `Hall` (ajuste fino no painel — seção 3 📡) → SOBRESSALENTE **DLY** `Sweet` (Mix 25 / Time 530 ms / Fdbk 26)
```
