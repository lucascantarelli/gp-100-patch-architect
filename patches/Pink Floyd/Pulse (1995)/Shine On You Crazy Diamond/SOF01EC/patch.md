...
> **⚡ Ajuste a Strat antes de tocar: SELETOR 2 · Volume 9 · Tone 8**
...
1. Toque a nota e CONTE 2 tempos antes da próxima
2. Bend na 4ª corda com vibrato lento
3. O volume da guitarra controla o brilho do eco

> 🚀 **Comece por aqui (3 passos)**:
> 1. Ajuste a guitarra conforme a tabela acima.
> 2. Carregue o patch no slot **U25** (seção 8).
> 3. Toque *intro inteira (parte I)* e confira o checklist da seção 5.
...
|---|---|
| eco embolando | toque mais devagar |
| eco baixo | DLY Mix +5 |
...
1. **Base seca (partes cantadas)** — **DLY → OFF**
   *Quando*: Entre a intro e os solos, toque os acordes sem eco.
   *Dica*: Religue o DLY para voltar ao som da intro.
...
**O que este patch é**: A intro icônica: cada nota ecoa uma vez — o delay É o riff (toque a cada 2 tempos).

A assinatura: intro com delay a cada nota (assinatura absoluta), base AOR e solo final.

**Teste recomendado**: intro inteira (parte I) · **Drum**: AOR lento

**Como saber que está certo**:

- ☑️ cada nota ecoada uma vez
- ☑️ espaço entre as notas preenchido pelo eco
...
|---|---|---|
| Intro: delay a cada nota tocada (assinatura); solo final com TD | alta | Pulse (1995) · gilmourish.com |
...
|---|---|---|---|---|---|---|---|---|
| ~~⚪~~ PRE | ~~⚪~~ DST | **🔴** AMP · Knights CL | ~~⚪~~ NR | **🔴** CAB · UK-LD 4x12 | ~~⚪~~ EQ | ~~⚪~~ MOD | **🔴** DLY · Sweet | **🔴** RVB · Hall |
...
| DLY | Sweet | Fdbk: 25 · Delay ms: 530 · High Cut: 26 |
| RVB | Hall | Decay*: 35 · Pre-D*: 45 · Damp*: 50 · Mix*: 1 |
...
**Nome no painel**: `SOF01EC` · **Slot sugerido**: **U25**

1. **GP-100 Edits (recomendado)**: conecte a GP-100 por USB → importe `<NOME>.prst` desta pasta → salve no slot **U25**.
2. **Digitar no painel** (receita na ordem dos menus):

```
**AMP** `Knights CL` (Gain 38 / Vol 62 / Bass 62 / Middle 48 / Treble 52 / p5 58) → **CAB** `UK-LD 4x12` (Level 78 / High Cut 62) → **DLY** `Sweet` (Fdbk 25 / Delay ms 530 / High Cut 26) → **RVB** `Hall` (ajuste fino no painel — seção 3 📡)
```

3. **SAVE** no slot → renomeie para `SOF01EC`.
