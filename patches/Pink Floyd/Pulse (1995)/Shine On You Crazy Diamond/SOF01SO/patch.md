...
1. Frases melódicas largas com bend longo
2. Espaço entre frases — o delay responde

> 🚀 **Comece por aqui (3 passos)**:
> 1. Ajuste a guitarra conforme a tabela acima.
> 2. Carregue o patch no slot **U27** (seção 8).
> 3. Toque *solo final (parte IX)* e confira o checklist da seção 5.
...
1. **Base seca** — **DLY → OFF**
   *Quando*: Refrões/acordes — eco some, sobra o sustain do amp.
...
**O que este patch é**: Solo final: Tube Driver quente com delay — a despedida de Syd em frases largas.

A assinatura: intro com delay a cada nota (assinatura absoluta), base AOR e solo final.

**Teste recomendado**: solo final (parte IX) · **Drum**: AOR lento

**Como saber que está certo**:

- ☑️ frases largas com sustain
- ☑️ eco preenchendo os espaços
...
| DLY | Sweet | Mix: 20 · Time: 530 ms · Fdbk: 28 |
...
**Nome no painel**: `SOF01SO` · **Slot sugerido**: **U27**

1. **GP-100 Edits (recomendado)**: conecte a GP-100 por USB → importe `<NOME>.prst` desta pasta → salve no slot **U27**.
2. **Digitar no painel** (receita na ordem dos menus):

```
**PRE** `Saturate` (Gain 50 / Mix 60 / Output 62) → **AMP** `Flagman` (Gain 50 / PRSE 58 / Master 64 / Bass 50 / Middle 55 / Treble 58) → **NR** `Gate 1` (Thr 30) → **CAB** `UK-LD 4x12` (Level 78 / High Cut 62) → **DLY** `Sweet` (Mix 20 / Time 530 ms / Fdbk 28) → **RVB** `Hall` (ajuste fino no painel — seção 3 📡)
```

3. **SAVE** no slot → renomeie para `SOF01SO`.
