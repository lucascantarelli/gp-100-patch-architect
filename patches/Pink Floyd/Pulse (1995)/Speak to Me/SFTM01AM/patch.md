...
> **⚡ Ajuste a Strat antes de tocar: SWELLS com a chave de volume · Seletor 2 · Tone 8**
...
1. Swell lento com a chave de volume (sem palhetada visível)
2. Deixe o delay acumular entre os swells

> 🚀 **Comece por aqui (3 passos)**:
> 1. Ajuste a guitarra conforme a tabela acima.
> 2. Carregue o patch no slot **U43** (seção 8).
> 3. Toque *texturas e swells com a chave de volume* e confira o checklist da seção 5.
...
|---|---|
| eco embolando | DLY Fdbk -10 |
| muito seco | RVB decay +10 |
...
**Nota específica deste patch**: Fábrica é o alvo: nenhum gabinete do banco casa melhor que o CAB já escolhido para este papel.
...
1. **Textura crua** — **DLY → OFF** + **RVB → OFF**
   *Quando*: Swell puro, sem ambiência (para camadas na gravação).
...
**O que este patch é**: Timbre ambiente: delay longo com feedback e reverb amplo — para texturas e swells da abertura.

Abertura com efeitos; sem guitarra — patch de ambiente para texturas e swells.

**Teste recomendado**: texturas e swells com a chave de volume · **Drum**: —

**Como saber que está certo**:

- ☑️ eco longo e respirável
- ☑️ sem lama nos graves
...
|---|---|---|
| Sem guitarra na faixa: só efeitos/ambient | alta | Wikipedia/Discogs (Pulse) · gilmourish.com |
...
|---|---|---|---|---|---|---|---|---|
| ~~⚪~~ PRE | ~~⚪~~ DST | **🔴** AMP · Flagman | ~~⚪~~ NR | **🔴** CAB · UK-LD 4x12 | ~~⚪~~ EQ | ~~⚪~~ MOD | **🔴** DLY · Sweet | **🔴** RVB · Hall |
...
**Nome no painel**: `SFTM01AM` · **Slot sugerido**: **U43**

1. **GP-100 Edits (recomendado)**: conecte a GP-100 por USB → importe `<NOME>.prst` desta pasta → salve no slot **U43**.
2. **Digitar no painel** (receita na ordem dos menus):

```
**AMP** `Flagman` (p0 25 / p1 50 / p2 55 / p3 50 / p4 50 / p5 55) → **CAB** `UK-LD 4x12` (p0 78 / p1 62) → **DLY** `Sweet` (Fdbk 45 / Delay ms 620 / High Cut 35) → **RVB** `Hall` (ajuste fino no painel — seção 3 📡)
```
