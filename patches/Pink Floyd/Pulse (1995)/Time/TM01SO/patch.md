...
1. Bend de 1 tom na 1ª e 2ª corda
2. Vibrato largo do pulso
3. Frases ascendentes com sustain

> 🚀 **Comece por aqui (3 passos)**:
> 1. Ajuste a guitarra conforme a tabela acima.
> 2. Carregue o patch no slot **U47** (seção 8).
...
1. **Verso seco** — **DLY → OFF**
   *Quando*: Acompanhamento do verso sem eco.
...
**O que este patch é**: O solo de Time: Tube Driver quente, delay curto, bend de 1 tom e a agressividade do relógio.

Solo de 'Time': TD + delay curto, bend de 1 tom e agressividade controlada — icônico.

**Teste recomendado**: solo central · **Drum**: DSOTM 121 BPM

**Como saber que está certo**:

- ☑️ agressividade controlada
- ☑️ bend de 1 tom com sustain
- ☑️ delay curto preenchendo
...
| AMP | Flagman | Gain: 52 · PRSE: 58 · Master: 64 · Bass: 50 · Middle: 55 · Treble: 58 |
| NR | Gate 1 | Thr: 30 |
| CAB | UK-LD 4x12 | Level: 78 · High Cut: 62 |
| DLY | Sweet | Mix: 18 · Time: 380 ms · Fdbk: 24 |
...
**Nome no painel**: `TM01SO` · **Slot sugerido**: **U47**

1. **GP-100 Edits (recomendado)**: conecte a GP-100 por USB → importe `<NOME>.prst` desta pasta → salve no slot **U47**.
2. **Digitar no painel** (receita na ordem dos menus):

```
**PRE** `Saturate` (Gain 50 / Mix 60 / Output 62) → **AMP** `Flagman` (Gain 52 / PRSE 58 / Master 64 / Bass 50 / Middle 55 / Treble 58) → **NR** `Gate 1` (Thr 30) → **CAB** `UK-LD 4x12` (Level 78 / High Cut 62) → **DLY** `Sweet` (Mix 18 / Time 380 ms / Fdbk 24) → **RVB** `Hall` (ajuste fino no painel — seção 3 📡)
```

3. **SAVE** no slot → renomeie para `TM01SO`.
