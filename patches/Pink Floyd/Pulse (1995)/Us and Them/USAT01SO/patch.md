...
1. Frases longas com sustain
2. Deixe o reverb completar o final das frases

> 🚀 **Comece por aqui (3 passos)**:
> 1. Ajuste a guitarra conforme a tabela acima.
> 2. Carregue o patch no slot **U53** (seção 8).
...
1. **Base (sem eco)** — **DLY → OFF**
   *Quando*: Acompanhamento discretíssimo.
...
**O que este patch é**: Solo aéreo: delay longo + reverb — as frases flutuando sobre a balada.

A balada épica: base limpa com chorus e reverb, solos aéreos com delay.

**Teste recomendado**: solo central · **Drum**: Balada épica

**Como saber que está certo**:

- ☑️ frases flutuando no reverb
- ☑️ eco preenchendo
...
**Nome no painel**: `USAT01SO` · **Slot sugerido**: **U53**

1. **GP-100 Edits (recomendado)**: conecte a GP-100 por USB → importe `<NOME>.prst` desta pasta → salve no slot **U53**.
2. **Digitar no painel** (receita na ordem dos menus):

```
**AMP** `Knights CL` (Gain 42 / Vol 62 / Bass 62 / Middle 48 / Treble 52) → **CAB** `UK-LD 4x12` (Level 78 / High Cut 62) → **DLY** `Sweet` (Mix 20 / Time 570 ms / Fdbk 30) → **RVB** `Hall` (ajuste fino no painel — seção 3 📡)
```

3. **SAVE** no slot → renomeie para `USAT01SO`.
