...
1. Palhetada alternada no riff — a base é percussiva
2. Dinâmica: mais forte = mais crunch (o Tube Driver responde)

> 🚀 **Comece por aqui (3 passos)**:
> 1. Ajuste a guitarra conforme a tabela acima.
> 2. Carregue o patch no slot **U29** (seção 8).
> 3. Toque *acordes do riff com palhetada alternada* e confira o checklist da seção 5.
...
1. **Solo (com eco)** — **DLY → ON**
   *Quando*: Ao chegar no solo, ligue o DLY: o eco do solo já está projetado.
...
**O que este patch é**: Base de crunch: Tube Driver leve empurrando o Hiwatt — o som de acorde que anda.

Blues rock direto: base com crunch e um dos solos mais 'SRV' do Gilmour.

**Teste recomendado**: acordes do riff com palhetada alternada · **Drum**: Blues rock 110 BPM

**Como saber que está certo**:

- ☑️ crunch que responde à palhetada
- ☑️ graves definidos
...
|---|---|---|
| Base crunch + solo bluesy com bend largo e vibrato largo | alta | Pulse (1995) · gilmourish.com |
...
|---|---|---|
| PRE | Saturate | Gain: 38 · Mix: 55 · Output: 62 |
...
**Nome no painel**: `WDF01BA` · **Slot sugerido**: **U29**

1. **GP-100 Edits (recomendado)**: conecte a GP-100 por USB → importe `<NOME>.prst` desta pasta → salve no slot **U29**.
2. **Digitar no painel** (receita na ordem dos menus):

```
**PRE** `Saturate` (Gain 38 / Mix 55 / Output 62) → **AMP** `Flagman` (Gain 44 / PRSE 55 / Master 62 / Bass 50 / Middle 55 / Treble 58) → **NR** `Gate 1` (Thr 26) → **CAB** `UK-LD 4x12` (Level 78 / High Cut 62) → **RVB** `Plate` (ajuste fino no painel — seção 3 📡) → SOBRESSALENTE **DLY** `Sweet` (Mix 15 / Time 430 ms / Fdbk 25)
```

3. **SAVE** no slot → renomeie para `WDF01BA`.
