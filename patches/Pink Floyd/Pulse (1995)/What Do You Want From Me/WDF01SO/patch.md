...
1. Bends de 1 tom inteiro com sustain
2. Frases com espaço — o delay preenche o vazio

> 🚀 **Comece por aqui (3 passos)**:
> 1. Ajuste a guitarra conforme a tabela acima.
> 2. Carregue o patch no slot **U30** (seção 8).
> 3. Toque *solo do meio e final* e confira o checklist da seção 5.
...
1. **Ritmo seco** — **DLY → OFF**
   *Quando*: Base do riff — acordes secos para marcar o groove.
...
**O que este patch é**: Solo bluesy: Tube Driver quente, bend largo de 1 tom, vibrato do pulso — Gilmour em modo SRV.

Blues rock direto: base com crunch e um dos solos mais 'SRV' do Gilmour.

**Teste recomendado**: solo do meio e final · **Drum**: Blues rock 110 BPM

**Como saber que está certo**:

- ☑️ sustain longo nos bends
- ☑️ definição nas notas rápidas
...
|---|---|---|
| PRE | Saturate | Gain: 50 · Mix: 60 · Output: 62 |
...
**Nome no painel**: `WDF01SO` · **Slot sugerido**: **U30**

1. **GP-100 Edits (recomendado)**: conecte a GP-100 por USB → importe `<NOME>.prst` desta pasta → salve no slot **U30**.
2. **Digitar no painel** (receita na ordem dos menus):

```
**PRE** `Saturate` (Gain 50 / Mix 60 / Output 62) → **AMP** `Flagman` (Gain 50 / PRSE 58 / Master 64 / Bass 50 / Middle 55 / Treble 58) → **NR** `Gate 1` (Thr 30) → **CAB** `UK-LD 4x12` (Level 78 / High Cut 62) → **DLY** `Sweet` (Mix 15 / Time 430 ms / Fdbk 25) → **RVB** `Hall` (ajuste fino no painel — seção 3 📡)
```

3. **SAVE** no slot → renomeie para `WDF01SO`.
