...
3. Atualizar o **fragmento do álbum (`data/defs/<CHAVE>.json`)** (campos `spec`/`doc` do patch) e rodar o pipeline — o `patch.md`, o `.prst` e o changelog do patch são gerados a partir dele; devolver a receita de digitação só dos itens alterados.
