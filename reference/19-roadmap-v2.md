...
| Pipeline | Reprodutível e guardado pela suíte — suíte em pirâmide pytest (`uv run pytest`; contagem derivada — badge `testes`): unit → integration → contract → e2e, com markers e auditoria de escrita no repo (`--guarda-repo`; issue #34), `TestH` de determinismo (derivados fora do git, ADR-0013) · CLI única do pacote `gp100` (`--json`; pipeline in-process desde a #33) |
