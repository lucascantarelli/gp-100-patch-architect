...
>
> **Atalho:** `gp100 build` (entry point do pacote) encadeia exatamente essa sequência, e `gp100 verify` roda a suíte. O shim `python gp100.py build` delega ao mesmo código.
...
| `gp100.py` | `python gp100.py find <termo>` · `show <NOME> [--json]` · `diff A B` · `export [--album ID] [--destino D]` · `build` · `verify` | **SHIM de transição (issue #48/#33)**: delega ao entry point `gp100` do pacote (`src/gp100_architect/interfaces/cli`) — mesmo código, menos um comando (`setlist`, abaixo). A CLI oficial já tem os 9 comandos de consulta e produção, com `--json` estável para automação e agentes. |
| `gp100_setlist.py` | `python gp100_setlist.py <música…> [--json]` | **SHIM de transição (issue #49/#33)**: delega ao `gp100 setlist` do pacote — cola de palco (ordenação por vizinho mais próximo, trocas ao entrar em cada música). Cálculo já é `domain/setlist` + `application/setlist`; o script fica só com argv e impressão. |
| `build_song_patches.py` | `python build_song_patches.py` | **Construtor principal.** Lê o defs (`tools/defs/`, schema v2) **validando-o primeiro** (loader + validador do domínio), escreve `patch.md` + `<NOME>.prst` (spec in-memory — ADR-0013) de cada patch em `patches/<Banda>/<Álbum>/<Música>/<NOME>/` e valida o resultado (XML conforme, nome ≤ 12 caracteres). Slots U01…Uxx calculados pela ordem global do defs. Com `--with-user-ir` (issue #10), escreve também a variante experimental `<NOME>-USERIR.prst` + `<NOME>-USERIR.md` de cada patch cujo gabinete tem captura no `ir_local` — o `.prst` já aponta para o slot de User IR (substituta direta do canônico; valide no aparelho antes de levar ao palco). Variantes NÃO são commitadas e ficam fora do guarda de determinismo (TestH). |
