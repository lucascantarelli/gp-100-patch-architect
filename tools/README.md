...
Rodam no CI a cada push (`.github/workflows/ci.yml`), que também roda o **pipeline de dados inteiro** e o `check_data_freshness.py` (job `dados`) e faz `tsc --noEmit` nos agentes. **Em push no `main`, o próprio job `dados` commita e envia os dados regenerados** (`chore: regenera os dados do pipeline [skip ci]`) — só em PR o passo de commit é pulado e o guarda reprova, pedindo que o autor rode o pipeline.
